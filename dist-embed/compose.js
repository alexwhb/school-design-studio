...
const G = 150, ze = {
  ...
  mm: G / 25.4,
  cm: G / 2.54
}, Ze = { px: 0, in: 2, mm: 1, cm: 2 };
function At(t, e) {
  const o = 10 ** Ze[e];
  return Math.round((Number(t) || 0) / ze[e] * o) / o;
}
const He = [
...
function We(t, e) {
  const i = At(t, "mm"), o = At(e, "mm"), r = Math.min(i, o), s = Math.max(i, o);
  return He.find((n) => Math.abs(n.mm[0] - r) <= vt && Math.abs(n.mm[1] - s) <= vt) ?? null;
}
function Te(t, e) {
  const i = We(t, e);
  return i ? `${i.name} ${Number(e) >= Number(t) ? "portrait" : "landscape"}` : null;
}
const P = "#ffffffff", Ge = "#000000ff", Ee = 0.6, St = 0.02, Re = 1.2, Ft = 96, Pe = 0.179;
function U(t) {
  if (typeof t != "string")
    return null;
...
function Y(t) {
  const e = U(t);
  return e ? 0.2126 * rt(e.r) + 0.7152 * rt(e.g) + 0.0722 * rt(e.b) : 0;
}
function S(t, e) {
...
function Ue(t) {
  return Te(t.width, t.height) ? G : Ft;
}
function Ye(t, e, i) {
  const o = (Number(t) || 0) * (Ft / Ue(i));
  return e ? o >= 18.66 : o >= 24;
}
function Je(t, e, i) {
  return Ye(t, e, i) ? 3 : 4.5;
}
const Dt = 3;
function ut(t, e) {
  let i = e[0], o = -1;
...
function Qt(t, e, i) {
  ...
  const s = i * Re, n = Fe(r), a = Y(e) > Pe ? -1 : 1;
  let l = t, d = o;
  for (let c = 1; c * St <= Ee; c++) {
    const h = n.l + a * c * St;
    if (h < 0 || h > 1)
      break;
    const g = gt({ ...Qe(n.h, n.s, h), a: r.a }), u = S(g, e);
    if (l = g, d = u, u >= s)
      return { color: g, ratio: u, met: !0, changed: !0 };
  }
  return { color: l, ratio: d, met: d >= i, changed: l !== t };
}
function Fe({ r: t, g: e, b: i }) {
  const o = t / 255, r = e / 255, s = i / 255, n = Math.max(o, r, s), a = Math.min(o, r, s), l = (n + a) / 2, d = n - a;
  if (d === 0)
    return { h: 0, s: 0, l };
  const c = d / (1 - Math.abs(2 * l - 1));
  let h = 0;
  return n === o ? h = (r - s) / d % 6 : n === r ? h = (s - o) / d + 2 : h = (o - r) / d + 4, { h: (h * 60 + 360) % 360, s: c, l };
}
function Qe(t, e, i) {
...
const Oe = "201", Ve = "Editorial slide — cover", Xe = 1920, $e = 1080, _e = "slide-themes", Ke = {
  ...
  brand: to,
  data: eo
}, oo = ["editorial", "swiss", "academic", "dark", "pastel"], ro = ["navy", "crimson", "forest"];
function no(t) {
...
const st = { alias: "Inter", id: 1, url: "/fonts/inter-400-700.woff2", value: "Inter" };
function Ct(t, e) {
...
function Ot(t, e) {
  const { background: i, layers: o } = no(e), r = E(i) || "#ffffffff", s = Ct(e, "primary") || "#1e3a5fff", n = Ct(e, "secondary") || s, a = o.filter((w) => w.type === "w-text"), l = (w) => S(w, r), d = [...new Set(a.map((w) => E(w.color)).filter((w) => !!w))].filter((w) => w !== s && w !== n).sort((w, x) => l(x) - l(w)), c = d[0] || "#111111ff", h = d.find((w) => w !== c && l(w) >= 3) || d[1] || c, g = o.filter((w) => w.type === "w-svg").flatMap((w) => w.colors || []).map(E).filter((w) => !!w && l(w) > 1.08).sort((w, x) => l(w) - l(x)), u = [...a].sort((w, x) => Number(x.fontSize) - Number(w.fontSize))[0], I = a.filter((w) => w.brandRole === "body").sort((w, x) => x.width - w.width)[0], y = a.find((w) => Number(w.fontSize) <= 28 && Number(w.letterSpacing) >= 6);
  return {
    key: t,
    paper: r,
    ink: c,
...
const so = {
  ...
  crimson: Vi,
  forest: io
}, lt = /* @__PURE__ */ new Map(), at = /* @__PURE__ */ new Map();
function Vt(t) {
  const e = oo.includes(String(t)) ? t : "editorial";
  return lt.has(e) || lt.set(e, Ot(e, so[e])), lt.get(e);
}
function Xt(t) {
  const e = ro.includes(String(t)) ? t : "navy";
  return at.has(e) || at.set(e, Ot(e, lo[e])), at.get(e);
}
const ao = /* @__PURE__ */ new Set([..."iljItf.,;:'\"|!`()[]{}-/\\ "]), co = /* @__PURE__ */ new Set([..."mwMW@%"]), ho = /* @__PURE__ */ new Set([..."ABCDEFGHKNOPQRSUVXYZ0123456789$&#"]);
function go(t) {
  return t === " " ? 0.28 : t === "i" || t === "l" || t === "j" || t === "." || t === "," ? 0.25 : ao.has(t) ? 0.31 : co.has(t) ? 0.86 : ho.has(t) ? 0.65 : 0.53;
}
const uo = {
  "IBM Plex Mono": 0.6,
  "JetBrains Mono": 0.6
...
function wt(t, e) {
  const i = e.fontFamily ? uo[e.fontFamily] : void 0, o = e.fontFamily && wo[e.fontFamily] || 1;
  let r = 0;
  if (i !== void 0) {
    for (const n of t)
      r += i;
  ...
  for (const n of t)
    r += go(n);
  const s = e.bold ? 1.03 : 1;
  return (r * e.fontSize * o * s + (e.letterSpacing || 0) * t.length) * fo;
}
function Nt(t, e, i) {
  const o = [];
...
function $t(t, e, i) {
  ...
  const r = (c) => ({ ...e, fontSize: c, letterSpacing: (e.letterSpacing || 0) * (c / e.fontSize) }), s = (c, h) => c.reduce((g, u) => Math.max(g, wt(u, h)), 0), n = Math.max(6, Math.min(i.minFontSize, e.fontSize));
  for (let c = e.fontSize; c >= n; c -= 1) {
    const h = r(c), g = Nt(o, i.width, h), u = Math.min(jt(i.height, c, e.lineHeight), i.maxLines || Number.MAX_SAFE_INTEGER);
    if (g.length <= u && s(g, h) <= i.width)
      return { text: o, fontSize: c, lines: g, truncated: !1 };
  }
  const a = r(n), l = Math.min(jt(i.height, n, e.lineHeight), i.maxLines || Number.MAX_SAFE_INTEGER), d = Nt(o, i.width, a).slice(0, l);
  for (; d.length && wt(`${d[d.length - 1]}…`, a) > i.width; ) {
    const c = d[d.length - 1], h = c.split(" ");
    if (h.length > 1) {
      h.pop(), d[d.length - 1] = h.join(" ");
      continue;
    }
    if (c.length <= 1) {
      d.pop();
    ...
    d[d.length - 1] = c.slice(0, -1);
  }
  return d.length === 0 ? { text: "…", fontSize: n, lines: ["…"], truncated: !0 } : (d[d.length - 1] = `${d[d.length - 1].replace(/[\s,;:.]+$/, "")}…`, { text: d.join(" "), fontSize: n, lines: d, truncated: !0 });
}
function _t(t, e) {
...
const yo = /^(address|blockquote|div|dl|dd|dt|h[1-6]|li|ol|p|pre|table|td|th|tr|ul)$/, xo = {
  amp: "&",
  lt: "<",
  gt: ">",
  quot: '"',
...
`).replace(/<\/?([a-z][a-z0-9]*)\b[^>]*>/gi, (i, o) => yo.test(String(o).toLowerCase()) ? `
` : "");
  return O(e).replace(/\n{3,}/g, `
...
function po(t) {
  return !t || !t.includes("{{") ? !1 : mt(t).search(Kt) !== -1;
}
function qt(t) {
  if (!t || !t.includes("{{"))
    return [];
  const e = [], i = /* @__PURE__ */ new Set();
  for (const o of mt(t).matchAll(Kt)) {
...
], Ao = 8, oe = ["name", "shortName", "tagline", "address", "phone", "email", "website"], re = [
...
function J(t) {
  return t ? ko.find((e) => e.id === t) : void 0;
}
function No(t, e) {
  switch (j(e)) {
    case "upper":
...
function ne(t) {
  ...
  return (r) => {
    ...
    if (a !== void 0)
      return n.reduce(No, a);
  };
}
function se(t) {
  const e = j(t.split("|")[0]);
  return re.some((i) => i.field === e);
}
function le(t) {
  const e = So();
  ...
  for (const s of i) {
    const n = Co(String(s));
    n && !e.colors.includes(n) && e.colors.length < Ao && e.colors.push(n);
  }
...
const jo = /^#?([0-9a-f]{6})([0-9a-f]{2})?$/i;
function Z(t) {
  const e = jo.exec((t || "").trim());
...
function ae(t, e, i) {
  const o = Z(e), r = Z(i);
  return !o || !r || o.alpha === "00" || o.rgb === r.rgb && o.alpha === r.alpha ? t : Lo(t, o, r);
}
function Bo(t, e) {
...
}, Ho = "circle at 50% 50%", Wo = (t) => /^(linear|radial)-gradient\(/.test(t?.trim() || "");
function To(t, e, i) {
  const o = i.map((r) => `${r.color} ${r.offset * 100}%`).join(",");
  return t === "radial" ? `radial-gradient(${Ho}, ${o})` : `linear-gradient(${e}deg, ${o})`;
}
function Go(t) {
  const e = [];
  let i = 0, o = 0;
...
function dt(t) {
  ...
  return o.forEach((a, l) => {
    const d = /\s(-?[\d.]+)%\s*$/.exec(a), c = d ? a.slice(0, d.index) : a, h = d ? Number(d[1]) / 100 : l / Math.max(1, o.length - 1);
    n.push({ color: Eo(c.trim()), offset: h });
...
function Uo(t, e) {
  return t.fontClass?.value === e.value ? !1 : (t.fontClass = Po(e), t.fontFamily = e.value, !0);
}
function m(t) {
  if (typeof t != "string")
    return null;
...
function C(t, e, i) {
  const o = e();
  if (typeof o != "string" || !Wo(o))
    return [{ kind: t, read: e, write: i }];
  const r = dt(o);
  return r ? r.stops.map((s, n) => ({
    kind: t,
    read: () => dt(String(e() ?? ""))?.stops[n]?.color,
    ...
    write: (a) => {
      ...
      if (!l)
        return;
      const d = l.stops.map((c, h) => h === n ? { ...c, color: a } : c);
      i(To(l.type, l.angle, d));
    }
  })) : [];
}
function Yo(t) {
  return Bo(t.textEffects, t.color).map((e) => ({
    kind: "layer",
    read: () => e.value,
    write: (i) => {
      t.textEffects = ae(JSON.parse(JSON.stringify(t.textEffects)), e.value, i);
    }
  }));
}
function ce(t) {
...
function L(t, e) {
  ...
  Array.isArray(i) && i.length && (t.textEffects = ae(JSON.parse(JSON.stringify(i)), t.color, e)), t.color = e;
}
function Jo(t) {
  const e = /* @__PURE__ */ new Map();
  for (const i of ce(t)) {
    const o = m(i.read());
    !o || _(o.rgb) || e.set(o.rgb, (e.get(o.rgb) || 0) + 1);
  }
  return [...e.entries()].sort((i, o) => o[1] - i[1]).map(([i]) => i);
}
const ft = /* @__PURE__ */ new Set([...$, "w-svg"]);
function B(t) {
...
function he(t, e, i, o) {
  const r = B(e[t]), s = (r.left + r.right) / 2, n = (r.top + r.bottom) / 2;
  let a = null, l = null;
  for (let c = 0; c < e.length; c++) {
    const h = e[c];
    c === t || h.hidden || h.isContainer || F(i[t], i[c]) && (l && !F(i[c], l) || Fo(B(h), s, n) && (a = h, l = i[c]));
  }
  if (!a)
    return o;
  if (!ft.has(a.type))
    return null;
  const d = m(It(a));
  return d ? d.alpha === "00" ? o : o ? { color: N(`#${d.rgb}${d.alpha}`, o.color), rgb: d.rgb, uuid: a.uuid, bounds: B(a) } : null : null;
}
function Vo(t) {
...
function _o(t, e, i) {
  ...
  if (!r.size)
    return o;
  const s = $o(t), n = Oo(t), a = Vo(e), l = t.map((h, g) => h.type === "w-text" && !h.hidden ? he(g, t, n, a) : null), d = /* @__PURE__ */ new Set();
  for (const h of l)
    h?.uuid && d.add(h.uuid);
  for (let h = 0; h < t.length; h++) {
    ...
    if (!I)
      continue;
    const y = r.has(I.rgb), w = r.has(u.rgb);
    if (!y && !w)
      continue;
    const x = Je(Number(g.fontSize) || 0, de(g), e), Be = N(`#${u.rgb}${u.alpha}`, I.color);
    if (S(Be, I.color) >= x)
      continue;
    const et = `#${u.rgb}${u.alpha}`;
    if (_(u.rgb)) {
      const ot = ut(I.color, [et, P, s]);
      ot !== et && (L(g, `#${m(ot).rgb}${u.alpha}`), o.swapped++), S(ot, I.color) < x && o.unreadable++;
      continue;
    }
    const D = Qt(et, I.color, x);
    if (D.met) {
      D.changed && (L(g, D.color), o.adjusted++);
      continue;
    }
    const it = ut(I.color, [D.color, P, s]);
    it === D.color ? D.changed && (L(g, D.color), o.adjusted++) : (L(g, `#${m(it).rgb}${u.alpha}`), o.swapped++), S(it, I.color) < x && o.unreadable++;
  }
  const c = qo(t, n, a, r, s, d);
...
const Ko = 0.25;
function qo(t, e, i, o, r, s) {
  const n = { nudged: 0, swapped: 0 };
  if (!i)
    return n;
  for (let a = 0; a < t.length; a++) {
    const l = t[a];
    if (l.hidden || l.isContainer || !ft.has(l.type) || l.type === "w-svg" && l.colors?.length !== 1)
      continue;
    const d = m(It(l));
    if (!d || d.alpha === "00")
      continue;
    const c = `#${d.rgb}${d.alpha}`, h = B(l);
    if (_(d.rgb)) {
      const y = he(a, t, e, i);
      if (!y || !o.has(y.rgb) || s.has(l.uuid) || Bt(h) > Ko * Bt(y.bounds) || S(N(c, y.color), y.color) >= Dt)
        continue;
      const w = ut(y.color, [c, P, r]);
      if (w === c)
        continue;
    ...
    for (let y = 0; y < t.length; y++) {
      const w = t[y];
      if (y === a || w.hidden || !ft.has(w.type) || !F(e[a], e[y]) || u && !F(e[y], u) || !Qo(B(w), h))
        continue;
      const x = m(It(w));
      !x || !o.has(x.rgb) || x.alpha === "00" || (g = N(`#${x.rgb}${x.alpha}`, i.color), u = e[y]);
    }
    if (!g || S(N(c, g), g) >= 1.5)
      continue;
    const I = Qt(c, g, Dt);
...
function t0(t, e, i) {
  const o = { filled: 0, fieldPages: 0, unresolved: 0, fonts: 0, recoloured: 0, backgrounds: 0, readability: ge() };
  if (i.fields) {
    const n = ne(e), a = /* @__PURE__ */ new Set();
    t.forEach((l, d) => {
      for (const c of l.layers) {
        if (!Lt(c) || !po(c.text))
          continue;
        const h = te(c.text, n);
        h !== c.text && (c.text = h, o.filled++, a.add(d));
      }
    }), o.fieldPages = a.size, o.unresolved = Mo(t.flatMap((l) => l.layers)).filter(se).length;
  }
  ...
  if (i.fonts && (r || s))
    for (const n of t) {
      ...
      for (const l of n.layers) {
        if (!Lt(l))
          continue;
        const c = de(l) || Number(l.fontSize) >= a ? r || s : s || r;
        c && Uo(l, c) && o.fonts++;
      }
    }
  if (i.colors && e.colors.length) {
    const n = Jo(t).slice(0, e.colors.length), a = /* @__PURE__ */ new Map();
    if (n.forEach((l, d) => {
      const c = m(e.colors[d]);
      c && c.rgb !== l && a.set(l, c.rgb);
    }), a.size) {
      for (const l of ce(t)) {
        const d = m(l.read()), c = d && a.get(d.rgb);
        !d || !c || (l.write(`#${c}${d.alpha}`), l.kind === "page" ? o.backgrounds++ : o.recoloured++);
      }
      for (const l of t) {
        const d = _o(l.layers, l.global, e);
...
function K(t, e) {
  ...
  return t0(i.layouts, le(e), { fields: !0, fonts: !0, colors: !0 }), i;
}
const f = 110, { width: ue, height: we } = R, p = ue - f * 2, v = we - 96, fe = ["title", "statement", "content", "two-column", "media"];
function yt(t) {
...
function k(t, e, i) {
  ...
  if (!o)
    return null;
  const r = $t(o, { fontFamily: i.font.value, fontSize: i.size, lineHeight: i.lineHeight, letterSpacing: i.tracking, bold: (i.weight || 400) >= 600 }, { width: e.width, height: e.height, minFontSize: i.minSize, maxLines: i.maxLines });
  if (!r.lines.length)
    return null;
  const s = _t(r, i.lineHeight);
  return {
    widget: Mt({
      left: e.left,
      top: e.top,
...
function bt(t, e, i, o) {
  ...
  for (const c of e) {
    ...
    if (!h || h.bottom > d)
      break;
    r.push(b(i.left + Math.round(o * 0.35), l + Math.round(o * 0.52), Math.round(o * 0.3), Math.round(o * 0.3), t.accent, 999)), r.push(h.widget), l = h.bottom + n;
    for (const g of c.sub || []) {
      const u = k(
        g,
...
function H(t, e) {
  const i = b(f, v, p, 2, t.rule), o = e("{{school.name|upper}}"), r = Mt({
    left: f,
    top: v + 22,
    width: p,
    height: 32,
...
function ye(t, e, i) {
  ...
  if (o.push(...bt(t, e.bullets, { left: f, top: r, width: p, height: v - 40 - n - r }, 32)), e.callout) {
    const a = v - 40 - n;
...
function o0(t, e, i) {
  const o = [], r = Ie(t, e, o), s = 80, n = Math.round((p - s) / 2), a = [
  ...
  for (const l of a) {
    ...
    c && (o.push(c.widget), d = c.bottom + 24), o.push(...bt(t, l.bullets, { left: l.left, top: d, width: n, height: v - 40 - d }, 28));
  }
  return [...o, ...H(t, i)];
}
function r0(t, e, i) {
  const o = [], r = Math.round(p * 0.46), s = { left: f + r + 70, top: 110, width: p - r - 70, height: v - 150 };
  e.image?.url ? o.push(bo(s.left, s.top, s.width, s.height, e.image)) : o.push(b(s.left, s.top, s.width, s.height, t.rule, 6));
  ...
  return d && (a = d.bottom + 30), d && o.push(d.widget), o.push(...bt(t, e.bullets, { left: f, top: a, width: r, height: v - 40 - a }, 26)), [...o, ...H(t, i)];
}
const n0 = {
  title: e0,
  statement: i0,
...
function xe(t, e, i = (o) => o) {
  const o = n0[t.layout] || ye, r = t.title?.trim() || t.kicker?.trim() || "Slide";
  return { global: ie(r.slice(0, 60), ue, we, e.paper, t.notes), layers: o(e, t, i) };
}
function ir(t, e = {}) {
  const i = Vt(e.theme), o = Q(e.brand), r = Array.isArray(t?.slides) ? t.slides : [], s = (r.length ? r : [yt("title")]).map((a) => xe({ ...yt(a?.layout || "content"), ...a }, i, o)), n = { format: "design-studio/v1", title: String(t?.title || "Untitled deck"), layouts: s };
  return e.brand ? K(n, e.brand) : n;
}
const pe = [
  {
    id: 19,
...
for (const t of pe)
  t.type === "svg" && (z.set(t.title.toLowerCase(), t), z.set(t.title.toLowerCase().replace(/\s+/g, "-"), t), z.set(t.title.toLowerCase().replace(/\s+/g, ""), t));
const or = pe.filter((t) => t.type === "svg").map((t) => t.title);
function me(t) {
  return !!t && z.has(String(t).trim().toLowerCase());
}
function kt(t, e, i, o, r) {
  const s = z.get(String(t).trim().toLowerCase());
  return s ? {
    name: s.title,
    type: "w-svg",
    uuid: V(),
...
function l0(t) {
  const e = s0[String(t?.size)], i = Zt[e ? e.size : t?.size] || Zt.letter;
...
function W(t, e, i, o, r) {
  const { W: s, H: n, M: a, content: l } = e, d = [], c = o ? t.paper : t.muted, h = Math.round(n * 0.07), g = M(
    i.eyebrow ? i.eyebrow.toUpperCase() : null,
    { left: a, top: h, width: l, height: Math.round(n * 0.05) },
    {
      font: t.eyebrow,
...
function d0(t, e, i, o) {
  ...
  let c = Math.round(s * 0.56);
  d && (l.push(d.widget), c = d.bottom + Math.round(s * 0.035));
  const h = kt("arrow right", n + Math.round((a - r * 0.22) / 2), c, Math.round(r * 0.22), t.accent);
  h && (l.push(h), c += Math.round(r * 0.22) + Math.round(s * 0.02));
  const g = M(i.sub, A(e, c, s * 0.86 - c), {
    font: t.body,
...
function c0(t, e, i, o) {
  ...
  let c = Math.round(s * 0.17);
  const h = me(i.icon) ? kt(i.icon, n + Math.round((a - d) / 2), c, d, t.accent) : null;
  h && (l.push(h), c += d + Math.round(s * 0.04));
  const g = M(i.head, A(e, c, s * 0.28), {
    font: t.display,
  ...
  g && (l.push(g.widget), c = g.bottom + Math.round(s * 0.03));
  const u = M(i.sub, A(e, c, s * 0.86 - c), {
    font: t.body,
...
function g0(t, e, i, o) {
  ...
  d && (l.push(d.widget), c = d.bottom + Math.round(s * 0.03)), l.push(b(n + Math.round(a * 0.35), c, Math.round(a * 0.3), 6, t.rule)), c += Math.round(s * 0.035);
  const h = M(i.badge ? i.head : i.sub, A(e, c, s * 0.2), {
    font: t.display,
  ...
  h && (l.push(h.widget), c = h.bottom + Math.round(s * 0.025));
  const g = M(i.badge ? i.sub : null, A(e, c, s * 0.86 - c), {
    font: t.body,
...
function be(t, e, i, o) {
  ...
  let c = Math.round(s * 0.46);
  d && (l.push(d.widget), c = d.bottom + Math.round(s * 0.03));
  const h = me(i.icon) ? kt(i.icon, e.W - n - Math.round(r * 0.14), Math.round(s * 0.17), Math.round(r * 0.14), t.accentSoft) : null;
  h && l.push(h);
  const g = M(i.sub, A(e, c, s * 0.86 - c), {
    font: t.body,
    size: Math.round(r * 0.04),
...
function ke(t, e, i, o = (r) => r) {
  const r = a0(i.width, i.height), s = u0[t.layout] || be, { layers: n, background: a } = s(e, r, t, o), l = t.head?.trim() || t.eyebrow?.trim() || "Sign";
  return { global: ie(l.slice(0, 60), i.width, i.height, a), layers: n };
}
function rr(t, e = {}) {
  const i = Xt(e.theme), o = Q(e.brand), r = l0(t || { orientation: "PORTRAIT", size: "letter" }), s = Array.isArray(t?.signs) ? t.signs : [], n = (s.length ? s : [xt("notice")]).map((l) => ke({ ...xt(l?.layout || "notice"), ...l }, i, r, o)), a = { format: "design-studio/v1", title: s[0]?.head?.trim() || "Untitled sign", layouts: n };
  return e.brand ? K(a, e.brand) : a;
}
function w0(t) {
  const e = qt(t.text), i = e.find((o) => se(o));
  ...
  return {
    ...
    pages: e.map((i, o) => {
      ...
      return {
        ...
        texts: r.filter((n) => n.type === "w-text" && !n.hidden).map((n) => ({ id: String(n.uuid), role: w0(n), text: mt(n.text) })).filter((n) => n.text.length > 0),
        images: r.filter((n) => n.type === "w-image" && !n.hidden).map((n) => ({ id: String(n.uuid), alt: f0(n) })),
        notes: s || null
      };
    })
  };
}
const I0 = 400, y0 = 420, x0 = "rise", Ae = /* @__PURE__ */ new Set(["bullet", "sub-bullet"]);
function ve(t) {
  return String(t.role ?? "");
}
function Ht(t, e) {
  const i = t[e], o = t[e + 1];
  return !i || !o ? !1 : String(i.type) === "w-svg" && Ae.has(ve(o));
}
function p0(t) {
  return t.map((e, i) => {
    const o = { ...e }, r = Ht(t, i), s = Ae.has(ve(e));
    if (!s && !r)
...
function M0(t, e) {
  ...
  return i.length === 0 || tt(t) !== "slides" ? t : {
    ...t,
    layouts: i.map((o) => ({
      ...o,
      global: e ? { ...o.global, transition: { type: "fade", duration: I0 } } : b0(o.global),
      layers: e ? p0(o.layers || []) : m0(o.layers || [])
    }))
  };
}
function b0(t) {
...
const Se = ["href", "color", "bold", "italic", "underline", "strike"], Wt = /^(ADDRESS|ARTICLE|BLOCKQUOTE|DIV|DL|DD|DT|FOOTER|H[1-6]|HEADER|LI|OL|P|PRE|SECTION|TABLE|TD|TH|TR|UL)$/, Tt = /^(HEAD|LINK|META|NOSCRIPT|SCRIPT|STYLE|TEMPLATE|TITLE)$/, Gt = 3, Et = 1, De = 64;
...
function pt(t) {
  return t.replace(/[&<>"\u00a0]/g, (e) => k0[e]);
}
const A0 = /^#([0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i, v0 = /^rgba?\(\s*([^)]+)\)$/i, T = (t) => Math.max(0, Math.min(255, Math.round(t))).toString(16).padStart(2, "0");
function Rt(t) {
  if (!t)
    return;
  const e = t.trim(), i = e.match(A0);
  if (i) {
...
const S0 = /^([a-z][a-z0-9+.-]*):/i, D0 = /* @__PURE__ */ new Set(["http", "https", "mailto", "tel"]);
function C0(t) {
  const e = (t ?? "").trim();
  if (!e)
    return;
  const i = e.match(S0)?.[1].toLowerCase();
  if (i)
    return D0.has(i) ? e : void 0;
  if (e.startsWith("//"))
    return "https:" + e;
  if (!/\s/.test(e))
    return "https://" + e;
...
function N0(t, e) {
  return Se.every((i) => (t[i] || void 0) === (e[i] || void 0));
}
function j0(t, e) {
  const i = { ...e };
  switch (t.tagName) {
    ...
    case "DEL":
      ...
      break;
    case "A": {
      const r = C0(t.getAttribute?.("href"));
      r && (i.href = r);
      break;
    }
    case "FONT": {
      const r = Rt(t.getAttribute?.("color"));
      r && (i.color = r);
      break;
    }
  }
...
function Ce(t) {
  ...
  const r = () => {
    ...
    let d = l;
    for (; ; ) {
      for (let h = d.nextSibling; h; h = h.nextSibling)
        if (h.nodeType === Gt && h.data || h.nodeType === Et && !Tt.test(h.tagName ?? ""))
          return !1;
      const c = d.parentNode;
      if (!c || c === t || Wt.test(c.tagName ?? ""))
        return !0;
      d = c;
    }
  }, n = (l, d) => {
    ...
    c && N0(c, d) ? c.text += l : i.push({ text: l, ...d }), o = !0;
  }, a = (l, d, c = 0) => {
    if (!(c > De))
...
function L0(t, e, i) {
  switch (t) {
    case "href":
      return `<a href="${pt(String(e))}">${i}</a>`;
    case "color":
      return `<span style="color:${pt(String(e))}">${i}</span>`;
    case "bold":
      return `<b>${i}</b>`;
    case "italic":
      return `<i>${i}</i>`;
...
function B0(t, e = "none") {
  if (e === "bullet" || e === "number") {
    const o = e === "number" ? "ol" : "ul", r = t.map((s) => `<li>${Pt(s).trim() ? Ut(s) : "<br>"}</li>`);
    return `<${o}>${r.join("")}</${o}>`;
  }
  const i = t.map(Ut).join("<br>");
  return t.length > 1 && !Pt(t[t.length - 1]) ? i + "<br>" : i;
}
const z0 = 3, je = 1, Z0 = De, H0 = /* @__PURE__ */ new Set(["AREA", "BASE", "BR", "COL", "EMBED", "HR", "IMG", "INPUT", "LINK", "META", "PARAM", "SOURCE", "TRACK", "WBR"]), W0 = /* @__PURE__ */ new Set(["SCRIPT", "STYLE", "TEXTAREA", "TITLE", "XMP", "IFRAME", "NOEMBED", "NOFRAMES"]), T0 = {
  LI: /^LI$/,
  P: /^P$/,
  DD: /^(DD|DT)$/,
  DT: /^(DD|DT)$/,
...
function G0(t) {
  return { nodeType: z0, data: t, childNodes: [], nextSibling: null, parentNode: null };
}
function E0(t) {
  if (!t)
    return null;
...
function Le(t) {
  const e = { nodeType: je, tagName: "BODY", childNodes: [], nextSibling: null, parentNode: null }, i = [e], o = () => i[i.length - 1], r = (l) => {
    const d = o(), c = d.childNodes[d.childNodes.length - 1];
    c && (c.nextSibling = l), l.parentNode = d, d.childNodes.push(l);
  }, s = (l) => {
    l && r(G0(O(l)));
  }, n = String(t ?? "");
  let a = 0;
  for (; a < n.length; ) {
    ...
    const c = d.name;
    if (d.close) {
      const u = i.findIndex((I) => I.tagName === c);
      u > 0 && (i.length = u);
      continue;
    }
    const h = T0[c];
    h && i.length > 1 && h.test(o().tagName ?? "") && i.pop();
    const g = U0(c, P0(d.attrs));
    if (r(g), W0.has(c)) {
      const u = n.toUpperCase().indexOf(`</${c}`, a);
      a = u < 0 ? n.length : n.indexOf(">", u) + 1 || n.length;
      continue;
    }
    !H0.has(c) && !d.selfClosing && i.length < Z0 && i.push(g);
  }
  return e;
}
function Q0(t, e = "none") {
  return B0(Ce(Le(String(t ?? ""))), e);
}
function lr(t) {
  return Ce(Le(String(t ?? ""))).map((e) => e.map((i) => i.text).join("")).join(`
`);
}
const ar = ["w-text", "w-image", "w-svg", "w-rect", "w-ellipse", "w-polygon", "w-path", "w-group", "w-qrcode", "w-table"], dr = "page", cr = {
...
}, O0 = /^[A-Za-z0-9 _-]{1,64}$/, V0 = {
  "w-text": ["fontClass.value"],
  "w-table": ["fontClass.value"]
};
function X0(t, e) {
  return typeof e != "string" ? !1 : t === "fontClass.value" ? O0.test(e) : !0;
}
function $0(t, e) {
  const i = e.split(".");
  let o = t;
...
function _0(t) {
  const e = JSON.parse(JSON.stringify(t)), i = { dropped: [] };
  for (const o of e.layouts || [])
    for (const r of o.layers || [])
      for (const s of V0[String(r.type)] || []) {
        const n = $0(r, s);
        if (!n)
          continue;
        const a = n.holder[n.key];
        a === void 0 || X0(s, a) || (i.dropped.push({ type: String(r.type), path: s, value: String(a).slice(0, 80) }), s === "fontClass.value" ? (delete r.fontClass, delete r.fontFamily) : delete n.holder[n.key]);
      }
  return { doc: e, report: i };
}
function K0(t) {
  return t === "poster" ? [...Me] : [...fe];
}
const Yt = 50;
function q0(t) {
  const { doc: e, report: i } = _0(t);
...
function Jt(t, e) {
  return e ? K({ format: "design-studio/v1", title: "", layouts: [t] }, e).layouts[0] : t;
}
function tr(t, e, i, o) {
...
`).map((n) => n.trim()).filter(Boolean).map((n) => ({ text: n, sub: [] }));
  if (tt(t) === "poster") {
    ...
    return Jt(ke(l, Xt(i.theme), a, Q(o)), o);
  }
  if (!fe.includes(e))
    return null;
  const s = {
...
`).map((n) => n.trim()).filter(Boolean).map((n) => ({ text: n, sub: [] }))
  ...
  return Jt(xe(s, Vt(i.theme), Q(o)), o);
}
function gr(t, e, i = {}) {
  let o = q0(t);
  ...
  for (const n of s) {
    ...
    switch (n.op) {
      ...
      case "setMarkup": {
        ...
        a.text = Q0(String(n.html ?? ""), a.listStyle);
        break;
      }
      case "setImage": {
        const a = ct(o, n.id);
        ...
        a.imgUrl = n.url;
        const c = d > l ? d / l : 1, h = d < l ? l / d : 1;
        a.zoom = c, a.zoomY = h, a.transform = ` scale(${c}, ${h}) translate(0px, 0px)`;
        break;
      }
      case "addPage": {
        if (o.layouts.length >= Yt) {
          r.push({ op: n, reason: `A design holds at most ${Yt} pages.` });
          break;
        }
        ...
        const l = tr(o, String(n.kind), n.fields || {}, i.brand);
        if (!l) {
          r.push({ op: n, reason: `“${n.kind}” is not a page this design can hold. Try one of: ${K0(tt(o) === "poster" ? "poster" : "slides").join(", ")}.` });
          break;
        }
...
export {
  fe as DECK_PAGE_KINDS,
  or as ICON_KEYS,
  hr as NESTED_URL_PATHS,
  dr as PAGE_TYPE,
  ro as POSTER_PACK_KEYS,
  ht as POSTER_PAGE,
  O0 as SAFE_FONT_FAMILY,
  V0 as SANITISED_FIELDS,
  Me as SIGN_PAGE_KINDS,
  R as SLIDE_PAGE,
  oo as SLIDE_THEME_KEYS,
  cr as URL_FIELDS,
  ar as WIDGET_TYPES,
  K as applyBrand,
  M0 as applyMotion,
  ...
  Le as parseMarkup,
  Xt as posterPack,
  _0 as sanitizeFields,
  Q0 as sanitizeMarkup,
  Vt as slideTheme
};
