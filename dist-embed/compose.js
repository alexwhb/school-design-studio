...
const G = 150, Ne = {
  ...
  mm: G / 25.4,
  cm: G / 2.54
}, je = { px: 0, in: 2, mm: 1, cm: 2 };
function kt(t, e) {
  const o = 10 ** je[e];
  return Math.round((Number(t) || 0) / Ne[e] * o) / o;
}
const Le = [
...
function ze(t, e) {
  ...
  return Le.find((n) => Math.abs(n.mm[0] - r) <= At && Math.abs(n.mm[1] - s) <= At) ?? null;
}
function Be(t, e) {
  const i = ze(t, e);
  return i ? `${i.name} ${Number(e) >= Number(t) ? "portrait" : "landscape"}` : null;
}
const P = "#ffffffff", Ze = "#000000ff", He = 0.6, vt = 0.02, We = 1.2, Yt = 96, Te = 0.179;
function U(t) {
  if (typeof t != "string")
    return null;
...
function Ge(t) {
  return Be(t.width, t.height) ? G : Yt;
}
function Ee(t, e, i) {
  const o = (Number(t) || 0) * (Yt / Ge(i));
  return e ? o >= 18.66 : o >= 24;
}
function Re(t, e, i) {
  return Ee(t, e, i) ? 3 : 4.5;
}
const St = 3;
function gt(t, e) {
  let i = e[0], o = -1;
...
function Jt(t, e, i) {
  ...
  const s = i * We, n = Pe(r), a = Y(e) > Te ? -1 : 1;
  let l = t, d = o;
  for (let h = 1; h * vt <= He; h++) {
    const c = n.l + a * h * vt;
    if (c < 0 || c > 1)
      break;
    const g = ct({ ...Ue(n.h, n.s, c), a: r.a }), u = S(g, e);
    if (l = g, d = u, u >= s)
      return { color: g, ratio: u, met: !0, changed: !0 };
  }
  return { color: l, ratio: d, met: d >= i, changed: l !== t };
}
function Pe({ r: t, g: e, b: i }) {
...
const Ye = "201", Je = "Editorial slide — cover", Fe = 1920, Qe = 1080, Oe = "slide-themes", Ve = {
  ...
  brand: hi,
  data: ci
}, ui = "216", wi = "Dark slide — cover", fi = 1920, Ii = 1080, yi = "slide-themes", xi = {
  colors: {
    "46cdb4": "primary"
  }
  ...
  brand: $i,
  data: Ki
}, qi = ["editorial", "swiss", "academic", "dark", "pastel"], to = ["navy", "crimson", "forest"];
function eo(t) {
...
function Ft(t, e) {
  const { background: i, layers: o } = eo(e), r = E(i) || "#ffffffff", s = Dt(e, "primary") || "#1e3a5fff", n = Dt(e, "secondary") || s, a = o.filter((w) => w.type === "w-text"), l = (w) => S(w, r), d = [...new Set(a.map((w) => E(w.color)).filter((w) => !!w))].filter((w) => w !== s && w !== n).sort((w, x) => l(x) - l(w)), h = d[0] || "#111111ff", c = d.find((w) => w !== h && l(w) >= 3) || d[1] || h, g = o.filter((w) => w.type === "w-svg").flatMap((w) => w.colors || []).map(E).filter((w) => !!w && l(w) > 1.08).sort((w, x) => l(w) - l(x)), u = [...a].sort((w, x) => Number(x.fontSize) - Number(w.fontSize))[0], f = a.filter((w) => w.brandRole === "body").sort((w, x) => x.width - w.width)[0], y = a.find((w) => Number(w.fontSize) <= 28 && Number(w.letterSpacing) >= 6);
  return {
    key: t,
    paper: r,
    ink: h,
...
function Qt(t) {
  const e = qi.includes(String(t)) ? t : "editorial";
  return st.has(e) || st.set(e, Ft(e, io[e])), st.get(e);
}
function Ot(t) {
  const e = to.includes(String(t)) ? t : "navy";
  return lt.has(e) || lt.set(e, Ft(e, oo[e])), lt.get(e);
}
const ro = /* @__PURE__ */ new Set([..."iljItf.,;:'\"|!`()[]{}-/\\ "]), no = /* @__PURE__ */ new Set([..."mwMW@%"]), so = /* @__PURE__ */ new Set([..."ABCDEFGHKNOPQRSUVXYZ0123456789$&#"]);
function lo(t) {
  return t === " " ? 0.28 : t === "i" || t === "l" || t === "j" || t === "." || t === "," ? 0.25 : ro.has(t) ? 0.31 : no.has(t) ? 0.86 : so.has(t) ? 0.65 : 0.53;
}
const ao = {
  Anton: 0.78,
  "Bebas Neue": 0.72,
...
function Vt(t, e) {
  const i = e.fontFamily && ao[e.fontFamily] || 1;
  let o = 0;
  for (const s of t)
    o += lo(s);
  const r = e.bold ? 1.03 : 1;
  return (o * e.fontSize * i * r + (e.letterSpacing || 0) * t.length) * ho;
}
function Ct(t, e, i) {
  const o = [];
...
const go = /^(address|blockquote|div|dl|dd|dt|h[1-6]|li|ol|p|pre|table|td|th|tr|ul)$/, uo = {
  amp: "&",
  lt: "<",
  gt: ">",
  quot: '"',
...
`).replace(/<\/?([a-z][a-z0-9]*)\b[^>]*>/gi, (i, o) => go.test(String(o).toLowerCase()) ? `
` : "");
  return O(e).replace(/\n{3,}/g, `
...
function wo(t) {
  return !t || !t.includes("{{") ? !1 : xt(t).search(Kt) !== -1;
}
function _t(t) {
  if (!t || !t.includes("{{"))
    return [];
  const e = [], i = /* @__PURE__ */ new Set();
  for (const o of xt(t).matchAll(Kt)) {
...
], po = 8, ie = ["name", "shortName", "tagline", "address", "phone", "email", "website"], oe = [
...
function J(t) {
  return t ? xo.find((e) => e.id === t) : void 0;
}
function Ao(t, e) {
  switch (j(e)) {
    case "upper":
...
function se(t) {
  ...
  for (const s of i) {
    const n = ko(String(s));
    n && !e.colors.includes(n) && e.colors.length < po && e.colors.push(n);
  }
...
const vo = /^#?([0-9a-f]{6})([0-9a-f]{2})?$/i;
function Z(t) {
  const e = vo.exec((t || "").trim());
...
function le(t, e, i) {
  const o = Z(e), r = Z(i);
  return !o || !r || o.alpha === "00" || o.rgb === r.rgb && o.alpha === r.alpha ? t : So(t, o, r);
}
function Do(t, e) {
...
}, jo = "circle at 50% 50%", Lo = (t) => /^(linear|radial)-gradient\(/.test(t?.trim() || "");
function zo(t, e, i) {
  const o = i.map((r) => `${r.color} ${r.offset * 100}%`).join(",");
  return t === "radial" ? `radial-gradient(${jo}, ${o})` : `linear-gradient(${e}deg, ${o})`;
}
function Bo(t) {
  const e = [];
  let i = 0, o = 0;
...
function at(t) {
  ...
  return o.forEach((a, l) => {
    ...
    n.push({ color: Zo(h.trim()), offset: c });
...
function To(t, e) {
  return t.fontClass?.value === e.value ? !1 : (t.fontClass = Wo(e), t.fontFamily = e.value, !0);
}
function m(t) {
  if (typeof t != "string")
    return null;
...
function C(t, e, i) {
  ...
  return r ? r.stops.map((s, n) => ({
    ...
    write: (a) => {
      ...
      i(zo(l.type, l.angle, d));
    }
  })) : [];
}
function Go(t) {
  return Do(t.textEffects, t.color).map((e) => ({
    kind: "layer",
    read: () => e.value,
    write: (i) => {
...
function he(t, e, i, o) {
  ...
  for (let h = 0; h < e.length; h++) {
    const c = e[h];
    h === t || c.hidden || c.isContainer || F(i[t], i[h]) && (l && !F(i[h], l) || Ro(z(c), s, n) && (a = c, l = i[h]));
  }
  if (!a)
    return o;
  if (!ut.has(a.type))
...
function Qo(t, e, i) {
  ...
  if (!r.size)
    return o;
  const s = Fo(t), n = Uo(t), a = Yo(e), l = t.map((c, g) => c.type === "w-text" && !c.hidden ? he(g, t, n, a) : null), d = /* @__PURE__ */ new Set();
  for (const c of l)
    c?.uuid && d.add(c.uuid);
  for (let c = 0; c < t.length; c++) {
    ...
    if (!f)
      continue;
    const y = r.has(f.rgb), w = r.has(u.rgb);
    if (!y && !w)
      continue;
    const x = Re(Number(g.fontSize) || 0, ae(g), e), Ce = N(`#${u.rgb}${u.alpha}`, f.color);
    if (S(Ce, f.color) >= x)
      continue;
    const tt = `#${u.rgb}${u.alpha}`;
    if (K(u.rgb)) {
      const it = gt(f.color, [tt, P, s]);
      it !== tt && (L(g, `#${m(it).rgb}${u.alpha}`), o.swapped++), S(it, f.color) < x && o.unreadable++;
      continue;
    }
    const D = Jt(tt, f.color, x);
    if (D.met) {
      D.changed && (L(g, D.color), o.adjusted++);
      continue;
    }
    const et = gt(f.color, [D.color, P, s]);
    et === D.color ? D.changed && (L(g, D.color), o.adjusted++) : (L(g, `#${m(et).rgb}${u.alpha}`), o.swapped++), S(et, f.color) < x && o.unreadable++;
  }
  const h = Vo(t, n, a, r, s, d);
...
function Vo(t, e, i, o, r, s) {
  ...
  for (let a = 0; a < t.length; a++) {
    ...
    if (K(d.rgb)) {
      const y = he(a, t, e, i);
      if (!y || !o.has(y.rgb) || s.has(l.uuid) || Lt(c) > Oo * Lt(y.bounds) || S(N(h, y.color), y.color) >= St)
        continue;
      const w = gt(y.color, [h, P, r]);
      if (w === h)
        continue;
    ...
    for (let y = 0; y < t.length; y++) {
      const w = t[y];
      if (y === a || w.hidden || !ut.has(w.type) || !F(e[a], e[y]) || u && !F(e[y], u) || !Po(z(w), c))
        continue;
      const x = m(wt(w));
    ...
    if (!g || S(N(h, g), g) >= 1.5)
      continue;
    const f = Jt(h, g, St);
    f.changed && (zt(l, f.color), n.nudged++);
  }
  return n;
}
function zt(t, e) {
...
function Xo(t, e, i) {
  ...
  if (i.fields) {
    ...
    }), o.fieldPages = a.size, o.unresolved = Io(t.flatMap((l) => l.layers)).filter(ne).length;
  }
  ...
  if (i.colors && e.colors.length) {
    const n = Eo(t).slice(0, e.colors.length), a = /* @__PURE__ */ new Map();
    if (n.forEach((l, d) => {
      ...
      for (const l of t) {
        const d = Qo(l.layers, l.global, e);
...
function _(t, e) {
  ...
  return Xo(i.layouts, se(e), { fields: !0, fonts: !0, colors: !0 }), i;
}
const I = 110, { width: ge, height: ue } = R, p = ge - I * 2, v = ue - 96, we = ["title", "statement", "content", "two-column", "media"];
function ft(t) {
...
function H(t, e) {
  const i = b(I, v, p, 2, t.rule), o = e("{{school.name|upper}}"), r = pt({
    left: I,
    top: v + 22,
    width: p,
    height: 32,
...
function $o(t, e, i) {
  ...
  const a = k(
    e.sub,
    { left: I, top: n, width: Math.round(p * 0.68), height: v - 40 - n },
    {
      font: t.body,
      size: 38,
      minSize: 26,
...
function Ko(t, e, i) {
  const o = [b(I, 250, 10, 420, t.accent)], r = I + 70, s = p - 70, n = q(t, e.kicker, 110);
  n && o.push(n.widget);
  const a = k(
    e.title || e.callout,
...
function fe(t, e, i, o = p) {
  const r = q(t, e.kicker, 110, I, o);
  let s = r ? r.bottom + 14 : 110;
  r && i.push(r.widget);
  const n = k(
    e.title,
    { left: I, top: s, width: o, height: 200 },
    {
      font: t.display,
      size: 62,
      minSize: 34,
  ...
  return n && (i.push(n.widget), s = n.bottom + 26), i.push(b(I, s, o, 2, t.rule)), s + 40;
}
function Ie(t, e, i) {
  const o = [];
  let r = fe(t, e, o);
  ...
  if (o.push(...mt(t, e.bullets, { left: I, top: r, width: p, height: v - 40 - n - r }, 32)), e.callout) {
    const a = v - 40 - n;
    o.push(b(I, a, p, n - 16, t.accent, 8));
    const l = k(
      e.callout,
      { left: I + 34, top: a + 28, width: p - 68, height: n - 72 },
      {
        font: t.body,
        size: 30,
        minSize: 20,
...
function _o(t, e, i) {
  const o = [], r = fe(t, e, o), s = 80, n = Math.round((p - s) / 2), a = [
    ...
    { left: I + n + s, head: e.columnHeads[1] || null, bullets: e.bulletsRight }
  ];
  o.push(b(I + n + Math.round(s / 2), r, 2, v - 40 - r, t.rule));
  for (const l of a) {
    let d = r;
    const h = k(
      l.head,
...
function qo(t, e, i) {
  ...
  return d && (a = d.bottom + 30), d && o.push(d.widget), o.push(...mt(t, e.bullets, { left: I, top: a, width: r, height: v - 40 - a }, 26)), [...o, ...H(t, i)];
}
const t0 = {
  title: $o,
  statement: Ko,
...
function ye(t, e, i = (o) => o) {
  const o = t0[t.layout] || Ie, r = t.title?.trim() || t.kicker?.trim() || "Slide";
  return { global: ee(r.slice(0, 60), ge, ue, e.paper, t.notes), layers: o(e, t, i) };
}
function G0(t, e = {}) {
  const i = Qt(e.theme), o = Q(e.brand), r = Array.isArray(t?.slides) ? t.slides : [], s = (r.length ? r : [ft("title")]).map((a) => ye({ ...ft(a?.layout || "content"), ...a }, i, o)), n = { format: "design-studio/v1", title: String(t?.title || "Untitled deck"), layouts: s };
...
const E0 = xe.filter((t) => t.type === "svg").map((t) => t.title);
function pe(t) {
...
function be(t, e, i, o = (r) => r) {
  const r = i0(i.width, i.height), s = l0[t.layout] || Me, { layers: n, background: a } = s(e, r, t, o), l = t.head?.trim() || t.eyebrow?.trim() || "Sign";
  return { global: ee(l.slice(0, 60), i.width, i.height, a), layers: n };
}
function R0(t, e = {}) {
  const i = Ot(e.theme), o = Q(e.brand), r = e0(t || { orientation: "PORTRAIT", size: "letter" }), s = Array.isArray(t?.signs) ? t.signs : [], n = (s.length ? s : [It("notice")]).map((l) => be({ ...It(l?.layout || "notice"), ...l }, i, r, o)), a = { format: "design-studio/v1", title: s[0]?.head?.trim() || "Untitled sign", layouts: n };
  ...
  return {
    ...
    pages: e.map((i, o) => {
      ...
      return {
        ...
        texts: r.filter((n) => n.type === "w-text" && !n.hidden).map((n) => ({ id: String(n.uuid), role: a0(n), text: xt(n.text) })).filter((n) => n.text.length > 0),
        images: r.filter((n) => n.type === "w-image" && !n.hidden).map((n) => ({ id: String(n.uuid), alt: d0(n) })),
        notes: s || null
      };
    })
  };
}
const ke = ["href", "color", "bold", "italic", "underline", "strike"], Zt = /^(ADDRESS|ARTICLE|BLOCKQUOTE|DIV|DL|DD|DT|FOOTER|H[1-6]|HEADER|LI|OL|P|PRE|SECTION|TABLE|TD|TH|TR|UL)$/, Ht = /^(HEAD|LINK|META|NOSCRIPT|SCRIPT|STYLE|TEMPLATE|TITLE)$/, Wt = 3, Tt = 1;
...
function yt(t) {
  return t.replace(/[&<>"\u00a0]/g, (e) => h0[e]);
}
const c0 = /^#([0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i, g0 = /^rgba?\(\s*([^)]+)\)$/i, T = (t) => Math.max(0, Math.min(255, Math.round(t))).toString(16).padStart(2, "0");
function Gt(t) {
  if (!t)
    return;
  const e = t.trim(), i = e.match(c0);
  if (i) {
...
const u0 = /^([a-z][a-z0-9+.-]*):/i, w0 = /* @__PURE__ */ new Set(["http", "https", "mailto", "tel"]);
function f0(t) {
  const e = (t ?? "").trim();
  if (!e)
    return;
  const i = e.match(u0)?.[1].toLowerCase();
  if (i)
    return w0.has(i) ? e : void 0;
  if (e.startsWith("//"))
    return "https:" + e;
  if (!/\s/.test(e))
    return "https://" + e;
...
function y0(t, e) {
  ...
  switch (t.tagName) {
    ...
    case "DEL":
      ...
      break;
    case "A": {
      const r = f0(t.getAttribute?.("href"));
      r && (i.href = r);
      break;
    }
    case "FONT": {
...
function Ae(t) {
  ...
  const r = () => {
    ...
    h && I0(h, d) ? h.text += l : i.push({ text: l, ...d }), o = !0;
  }, a = (l, d) => {
    for (const h of Array.from(l.childNodes)) {
      if (h.nodeType === Wt) {
        const g = h.data;
        if (!g || /^(UL|OL)$/.test(l.tagName ?? "") && !g.trim())
          continue;
        g.split(`
`).forEach((f, y) => {
          y > 0 && r(), n(f, d), y > 0 && !f && (o = !0);
        });
        continue;
      }
      if (h.nodeType !== Tt)
...
function x0(t, e, i) {
  switch (t) {
    case "href":
      return `<a href="${yt(String(e))}">${i}</a>`;
    case "color":
      return `<span style="color:${yt(String(e))}">${i}</span>`;
    case "bold":
      return `<b>${i}</b>`;
    case "italic":
      return `<i>${i}</i>`;
...
const m0 = 3, Se = 1, M0 = 64, b0 = /* @__PURE__ */ new Set(["AREA", "BASE", "BR", "COL", "EMBED", "HR", "IMG", "INPUT", "LINK", "META", "PARAM", "SOURCE", "TRACK", "WBR"]), k0 = /* @__PURE__ */ new Set(["SCRIPT", "STYLE", "TEXTAREA", "TITLE", "XMP", "IFRAME", "NOEMBED", "NOFRAMES"]), A0 = {
  LI: /^LI$/,
  P: /^P$/,
  DD: /^(DD|DT)$/,
  DT: /^(DD|DT)$/,
...
function v0(t) {
  return { nodeType: m0, data: t, childNodes: [], nextSibling: null, parentNode: null };
}
function S0(t) {
  if (!t)
    return null;
...
function De(t) {
  const e = { nodeType: Se, tagName: "BODY", childNodes: [], nextSibling: null, parentNode: null }, i = [e], o = () => i[i.length - 1], r = (l) => {
  ...
  for (; a < n.length; ) {
    ...
    const h = d.name;
    if (d.close) {
      const u = i.findIndex((f) => f.tagName === h);
      u > 0 && (i.length = u);
      continue;
    }
    const c = A0[h];
    c && i.length > 1 && c.test(o().tagName ?? "") && i.pop();
    const g = N0(h, C0(d.attrs));
    if (r(g), k0.has(h)) {
      const u = n.toUpperCase().indexOf(`</${h}`, a);
      a = u < 0 ? n.length : n.indexOf(">", u) + 1 || n.length;
      continue;
    }
    !b0.has(h) && !d.selfClosing && i.length < M0 && i.push(g);
  }
  return e;
}
function B0(t, e = "none") {
  return p0(Ae(De(String(t ?? ""))), e);
}
function U0(t) {
  return Ae(De(String(t ?? ""))).map((e) => e.map((i) => i.text).join("")).join(`
`);
}
function Z0(t) {
...
`).map((n) => n.trim()).filter(Boolean).map((n) => ({ text: n, sub: [] }))
  ...
  return Ut(ye(s, Qt(i.theme), Q(o)), o);
}
function Y0(t, e, i = {}) {
  let o = H0(t);
  ...
  for (const n of s) {
    ...
    switch (n.op) {
      ...
      case "setMarkup": {
        ...
        a.text = B0(String(n.html ?? ""), a.listStyle);
        break;
      }
      case "setImage": {
        const a = dt(o, n.id);
      ...
      case "addPage": {
        ...
        const l = W0(o, String(n.kind), n.fields || {}, i.brand);
        if (!l) {
          r.push({ op: n, reason: `“${n.kind}” is not a page this design can hold. Try one of: ${Z0(bt(o) === "poster" ? "poster" : "slides").join(", ")}.` });
          break;
        }
...
export {
  ...
  E0 as ICON_KEYS,
  to as POSTER_PACK_KEYS,
  ht as POSTER_PAGE,
  me as SIGN_PAGE_KINDS,
  R as SLIDE_PAGE,
  qi as SLIDE_THEME_KEYS,
  _ as applyBrand,
  Y0 as applyOps,
  It as blankSign,
  ft as blankSlide,
