const R = { width: 1920, height: 1080 }, lt = { width: 1275, height: 1650 };
function t0(t) {
  return t === "poster" ? { ...lt } : { ...R };
}
const G = 150, Ie = {
  px: 1,
  in: G,
  mm: G / 25.4,
  cm: G / 2.54
}, ye = { px: 0, in: 2, mm: 1, cm: 2 };
function pt(t, e) {
  const o = 10 ** ye[e];
  return Math.round((Number(t) || 0) / Ie[e] * o) / o;
}
const xe = [
...
function fe(t, e) {
  const i = pt(t, "mm"), o = pt(e, "mm"), r = Math.min(i, o), n = Math.max(i, o);
  return xe.find((s) => Math.abs(s.mm[0] - r) <= mt && Math.abs(s.mm[1] - n) <= mt) ?? null;
}
function pe(t, e) {
  const i = fe(t, e);
  return i ? `${i.name} ${Number(e) >= Number(t) ? "portrait" : "landscape"}` : null;
}
const E = "#ffffffff", me = "#000000ff", Me = 0.6, Mt = 0.02, be = 1.2, Bt = 96, ke = 0.179;
function P(t) {
  if (typeof t != "string")
    return null;
...
function U(t) {
  const e = P(t);
  return e ? 0.2126 * et(e.r) + 0.7152 * et(e.g) + 0.0722 * et(e.b) : 0;
}
function S(t, e) {
  const i = U(t), o = U(e), r = Math.max(i, o), n = Math.min(i, o);
...
function Ae(t) {
  return pe(t.width, t.height) ? G : Bt;
}
function ve(t, e, i) {
  const o = (Number(t) || 0) * (Bt / Ae(i));
  return e ? o >= 18.66 : o >= 24;
}
function Se(t, e, i) {
  return ve(t, e, i) ? 3 : 4.5;
}
const bt = 3;
function dt(t, e) {
  let i = e[0], o = -1;
...
function Zt(t, e, i) {
  ...
  const n = i * be, s = De(r), a = U(e) > ke ? -1 : 1;
  let l = t, d = o;
  for (let h = 1; h * Mt <= Me; h++) {
    const c = s.l + a * h * Mt;
    if (c < 0 || c > 1)
      break;
    const g = at({ ...Ce(s.h, s.s, c), a: r.a }), w = S(g, e);
    if (l = g, d = w, w >= n)
      return { color: g, ratio: w, met: !0, changed: !0 };
  }
  return { color: l, ratio: d, met: d >= i, changed: l !== t };
}
function De({ r: t, g: e, b: i }) {
  const o = t / 255, r = e / 255, n = i / 255, s = Math.max(o, r, n), a = Math.min(o, r, n), l = (s + a) / 2, d = s - a;
  if (d === 0)
    return { h: 0, s: 0, l };
  const h = d / (1 - Math.abs(2 * l - 1));
  let c = 0;
  return s === o ? c = (r - n) / d % 6 : s === r ? c = (n - o) / d + 2 : c = (o - r) / d + 4, { h: (c * 60 + 360) % 360, s: h, l };
}
function Ce(t, e, i) {
  const o = (1 - Math.abs(2 * i - 1)) * e, r = o * (1 - Math.abs(t / 60 % 2 - 1)), n = i - o / 2, s = Math.floor((t % 360 + 360) % 360 / 60), [a, l, d] = [
    [o, r, 0],
    [r, o, 0],
    [0, o, r],
    [0, r, o],
...
const Ne = "201", je = "Editorial slide — cover", Le = 1920, ze = 1080, Be = "slide-themes", Ze = {
  ...
  brand: Wi,
  data: Gi
}, Ri = ["editorial", "swiss", "academic", "dark", "pastel"], Ei = ["navy", "crimson", "forest"];
function Pi(t) {
...
const ot = { alias: "Inter", id: 1, url: "/fonts/inter-400-700.woff2", value: "Inter" };
function kt(t, e) {
...
function Ht(t, e) {
  const { background: i, layers: o } = Pi(e), r = T(i) || "#ffffffff", n = kt(e, "primary") || "#1e3a5fff", s = kt(e, "secondary") || n, a = o.filter((u) => u.type === "w-text"), l = (u) => S(u, r), d = [...new Set(a.map((u) => T(u.color)).filter((u) => !!u))].filter((u) => u !== n && u !== s).sort((u, x) => l(x) - l(u)), h = d[0] || "#111111ff", c = d.find((u) => u !== h && l(u) >= 3) || d[1] || h, g = o.filter((u) => u.type === "w-svg").flatMap((u) => u.colors || []).map(T).filter((u) => !!u && l(u) > 1.08).sort((u, x) => l(u) - l(x)), w = [...a].sort((u, x) => Number(x.fontSize) - Number(u.fontSize))[0], f = a.filter((u) => u.brandRole === "body").sort((u, x) => x.width - u.width)[0], y = a.find((u) => Number(u.fontSize) <= 28 && Number(u.letterSpacing) >= 6);
  return {
    key: t,
    paper: r,
    ink: h,
    ...
    eyebrowTracking: Number(y?.letterSpacing) || 8
  };
}
const Ui = {
  editorial: We,
  ...
  crimson: ji,
  forest: Ti
}, rt = /* @__PURE__ */ new Map(), nt = /* @__PURE__ */ new Map();
function Wt(t) {
  const e = Ri.includes(String(t)) ? t : "editorial";
  return rt.has(e) || rt.set(e, Ht(e, Ui[e])), rt.get(e);
}
function Gt(t) {
  const e = Ei.includes(String(t)) ? t : "navy";
  return nt.has(e) || nt.set(e, Ht(e, Yi[e])), nt.get(e);
}
const Ji = /* @__PURE__ */ new Set([..."iljItf.,;:'\"|!`()[]{}-/\\ "]), Fi = /* @__PURE__ */ new Set([..."mwMW@%"]), Qi = /* @__PURE__ */ new Set([..."ABCDEFGHKNOPQRSUVXYZ0123456789$&#"]);
function Vi(t) {
  return t === " " ? 0.28 : t === "i" || t === "l" || t === "j" || t === "." || t === "," ? 0.25 : Ji.has(t) ? 0.31 : Fi.has(t) ? 0.86 : Qi.has(t) ? 0.65 : 0.53;
}
const Oi = {
  Anton: 0.78,
  "Bebas Neue": 0.72,
...
function Tt(t, e) {
  const i = e.fontFamily && Oi[e.fontFamily] || 1;
  let o = 0;
  for (const n of t)
    o += Vi(n);
  const r = e.bold ? 1.03 : 1;
  return (o * e.fontSize * i * r + (e.letterSpacing || 0) * t.length) * Xi;
}
function At(t, e, i) {
  const o = [];
...
function Rt(t, e, i) {
  ...
  for (let l = e.fontSize; l >= r; l -= 1) {
    const d = { ...e, fontSize: l, letterSpacing: (e.letterSpacing || 0) * (l / e.fontSize) }, h = At(o, i.width, d), c = Math.min(vt(i.height, l, e.lineHeight), i.maxLines || Number.MAX_SAFE_INTEGER);
    if (h.length <= c)
      return { text: o, fontSize: l, lines: h, truncated: !1 };
  }
  const n = { ...e, fontSize: r, letterSpacing: (e.letterSpacing || 0) * (r / e.fontSize) }, s = Math.min(vt(i.height, r, e.lineHeight), i.maxLines || Number.MAX_SAFE_INTEGER), a = At(o, i.width, n).slice(0, s);
  for (; a.length && Tt(`${a[a.length - 1]}…`, n) > i.width; ) {
...
const Ki = /^(address|blockquote|div|dl|dd|dt|h[1-6]|li|ol|p|pre|table|td|th|tr|ul)$/, _i = {
  amp: "&",
  lt: "<",
  gt: ">",
  quot: '"',
...
`).replace(/<\/?([a-z][a-z0-9]*)\b[^>]*>/gi, (i, o) => Ki.test(String(o).toLowerCase()) ? `
` : "");
  return Ut(e).replace(/\n{3,}/g, `
...
function qi(t) {
  return !t || !t.includes("{{") ? !1 : wt(t).search(Pt) !== -1;
}
function Yt(t) {
  if (!t || !t.includes("{{"))
    return [];
  const e = [], i = /* @__PURE__ */ new Set();
  for (const o of wt(t).matchAll(Pt)) {
    const r = o[1].trim(), n = j(r);
...
function Jt(t, e) {
  return !t || !t.includes("{{") ? t ?? "" : t.replace(/\{\{([^{}]*?)\}\}/g, (i, o) => {
    const r = Ut(String(o).replace(/<[^>]*>/g, "")).trim();
    if (!r || /\n/.test(r))
      return i;
    const n = e(r);
    return n === void 0 ? i : Ft(n);
  });
}
function Ft(t) {
...
function b(t, e, i, o, r, n = 0) {
  const s = Math.max(1, Math.round(i)), a = Math.max(1, Math.round(o));
  return {
    name: "Shape",
    type: "w-svg",
    uuid: Q(),
    ...
    opacity: 1,
    parent: "-1",
    svgUrl: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${s} ${a}" preserveAspectRatio="none"><rect x="0" y="0" width="${s}" height="${a}" rx="${n}" fill="{{colors[0]}}"/></svg>`,
    setting: [],
    record: { width: 0, height: 0, minWidth: 10, minHeight: 10 }
  };
}
function io(t, e, i, o, r) {
  const n = i / o, s = r.width && r.height ? r.width / r.height : n, a = s > n ? s / n : 1, l = s < n ? n / s : 1;
  return {
    name: "Image",
    type: "w-image",
    uuid: Q(),
...
], ro = 8, Vt = ["name", "shortName", "tagline", "address", "phone", "email", "website"], Ot = [
...
function Y(t) {
  return t ? oo.find((e) => e.id === t) : void 0;
}
function ho(t, e) {
  switch (j(e)) {
    case "upper":
...
function Xt(t) {
  const e = lo(t) ? t : no, i = {};
  for (const { field: r, key: n } of Ot) {
    const s = (e[n] || "").trim();
    i[r] = s || void 0;
  }
  const o = $i(i);
  return (r) => {
    const [n, ...s] = r.split("|"), a = o(n);
    if (a !== void 0)
      return s.reduce(ho, a);
  };
}
function $t(t) {
  const e = j(t.split("|")[0]);
  return Ot.some((i) => i.field === e);
}
function Kt(t) {
  const e = so();
  ...
  for (const n of i) {
    const s = ao(String(n));
    s && !e.colors.includes(s) && e.colors.length < ro && e.colors.push(s);
  }
  const o = t.fonts && typeof t.fonts == "object" ? t.fonts : {};
  Y(o.heading) && (e.fonts.heading = o.heading), Y(o.body) && (e.fonts.body = o.body);
  const r = t.logo;
  return r && typeof r.url == "string" && r.url && (e.logo = { url: r.url, width: Number(r.width) || 0, height: Number(r.height) || 0 }), e;
}
const co = /^#?([0-9a-f]{6})([0-9a-f]{2})?$/i;
function Z(t) {
  const e = co.exec((t || "").trim());
...
function go(t, e, i) {
  ...
  return t.map((r) => {
    const n = { ...r };
    if (n.filling) {
      const s = { ...n.filling }, a = s.gradient?.stops;
      a && (s.gradient = {
        ...s.gradient,
      ...
      if (l?.colors) {
        ...
        s.imageContent = { ...s.imageContent, pattern: { ...l, colors: d } }, s.color = d[0];
      } else
        s.color = o(s.color);
      n.filling = s;
    }
...
function _t(t, e, i) {
  const o = Z(e), r = Z(i);
  return !o || !r || o.alpha === "00" || o.rgb === r.rgb && o.alpha === r.alpha ? t : go(t, o, r);
}
function uo(t, e) {
  const i = Array.isArray(t) ? t : [], o = Z(e), r = o && o.alpha !== "00" ? o.rgb : null, n = /* @__PURE__ */ new Map(), s = (a) => {
    const l = Z(a);
    if (!l || l.alpha === "00" || l.rgb === r)
      return;
    const d = n.get(l.rgb);
    (!d || parseInt(l.alpha, 16) > parseInt(d, 16)) && n.set(l.rgb, l.alpha);
  };
  for (const a of [...i].reverse()) {
    ...
    a.stroke?.enable && s(a.stroke.color), a.shadow?.enable && s(a.shadow.color);
  }
  return [...n].map(([a, l]) => ({ rgb: a, alpha: l, value: `#${a}${l}` }));
}
const wo = (t, e, i) => {
...
}, yo = "circle at 50% 50%", xo = (t) => /^(linear|radial)-gradient\(/.test(t?.trim() || "");
function fo(t, e, i) {
  const o = i.map((r) => `${r.color} ${r.offset * 100}%`).join(",");
  return t === "radial" ? `radial-gradient(${yo}, ${o})` : `linear-gradient(${e}deg, ${o})`;
}
function po(t) {
  const e = [];
  let i = 0, o = 0;
...
function ko(t, e) {
  return t.fontClass?.value === e.value ? !1 : (t.fontClass = bo(e), t.fontFamily = e.value, !0);
}
function m(t) {
  if (typeof t != "string")
    return null;
...
function C(t, e, i) {
  const o = e();
  if (typeof o != "string" || !xo(o))
    return [{ kind: t, read: e, write: i }];
  const r = st(o);
  return r ? r.stops.map((n, s) => ({
    kind: t,
    read: () => st(String(e() ?? ""))?.stops[s]?.color,
    ...
    write: (a) => {
      ...
      if (!l)
        return;
      const d = l.stops.map((h, c) => c === s ? { ...h, color: a } : h);
      i(fo(l.type, l.angle, d));
    }
  })) : [];
}
function Ao(t) {
  return uo(t.textEffects, t.color).map((e) => ({
    kind: "layer",
    read: () => e.value,
    write: (i) => {
      t.textEffects = _t(JSON.parse(JSON.stringify(t.textEffects)), e.value, i);
    }
  }));
}
function te(t) {
  ...
  for (const i of t) {
    ...
    for (const r of i.layers)
      ...
      ), e.push(...Ao(r))) : O.has(r.type) ? e.push(
        ...C(
          "layer",
          () => r.color,
          (n) => r.color = n
...
function L(t, e) {
  ...
  Array.isArray(i) && i.length && (t.textEffects = _t(JSON.parse(JSON.stringify(i)), t.color, e)), t.color = e;
}
function vo(t) {
  const e = /* @__PURE__ */ new Map();
  for (const i of te(t)) {
    const o = m(i.read());
    !o || X(o.rgb) || e.set(o.rgb, (e.get(o.rgb) || 0) + 1);
  }
  return [...e.entries()].sort((i, o) => o[1] - i[1]).map(([i]) => i);
}
const ht = /* @__PURE__ */ new Set([...O, "w-svg"]);
function z(t) {
...
function ee(t, e, i, o) {
  const r = z(e[t]), n = (r.left + r.right) / 2, s = (r.top + r.bottom) / 2;
  let a = null, l = null;
  for (let h = 0; h < e.length; h++) {
    const c = e[h];
    h === t || c.hidden || c.isContainer || J(i[t], i[h]) && (l && !J(i[h], l) || So(z(c), n, s) && (a = c, l = i[h]));
  }
  if (!a)
    return o;
  if (!ht.has(a.type))
...
function No(t) {
  ...
  return { color: N(`#${e.rgb}${e.alpha}`, E), rgb: e.rgb, uuid: "", bounds: i };
}
const jo = 0.5;
function Lo(t) {
  let e = null, i = jo;
...
function zo(t, e, i) {
  ...
  if (!r.size)
    return o;
  const n = Lo(t), s = Co(t), a = No(e), l = t.map((c, g) => c.type === "w-text" && !c.hidden ? ee(g, t, s, a) : null), d = /* @__PURE__ */ new Set();
  for (const c of l)
    c?.uuid && d.add(c.uuid);
  for (let c = 0; c < t.length; c++) {
    ...
    if (!f)
      continue;
    const y = r.has(f.rgb), u = r.has(w.rgb);
    if (!y && !u)
      continue;
    const x = Se(Number(g.fontSize) || 0, qt(g), e), we = N(`#${w.rgb}${w.alpha}`, f.color);
    if (S(we, f.color) >= x)
      continue;
    const _ = `#${w.rgb}${w.alpha}`;
    if (X(w.rgb)) {
      const tt = dt(f.color, [_, E, n]);
      tt !== _ && (L(g, `#${m(tt).rgb}${w.alpha}`), o.swapped++), S(tt, f.color) < x && o.unreadable++;
      continue;
    }
    const D = Zt(_, f.color, x);
    if (D.met) {
      D.changed && (L(g, D.color), o.adjusted++);
      continue;
    }
    const q = dt(f.color, [D.color, E, n]);
    q === D.color ? D.changed && (L(g, D.color), o.adjusted++) : (L(g, `#${m(q).rgb}${w.alpha}`), o.swapped++), S(q, f.color) < x && o.unreadable++;
  }
  const h = Zo(t, s, a, r, n, d);
...
const Bo = 0.25;
function Zo(t, e, i, o, r, n) {
  const s = { nudged: 0, swapped: 0 };
  if (!i)
    return s;
  for (let a = 0; a < t.length; a++) {
    const l = t[a];
    if (l.hidden || l.isContainer || !ht.has(l.type) || l.type === "w-svg" && l.colors?.length !== 1)
      continue;
    const d = m(ct(l));
    if (!d || d.alpha === "00")
      continue;
    ...
    if (X(d.rgb)) {
      const y = ee(a, t, e, i);
      if (!y || !o.has(y.rgb) || n.has(l.uuid) || Dt(c) > Bo * Dt(y.bounds) || S(N(h, y.color), y.color) >= bt)
        continue;
      const u = dt(y.color, [h, E, r]);
      if (u === h)
        continue;
      Ct(l, `#${m(u).rgb}${d.alpha}`), s.swapped++;
      continue;
    }
    if (!o.has(d.rgb))
      continue;
    ...
    for (let y = 0; y < t.length; y++) {
      const u = t[y];
      if (y === a || u.hidden || !ht.has(u.type) || !J(e[a], e[y]) || w && !J(e[y], w) || !Do(z(u), c))
        continue;
      const x = m(ct(u));
      !x || !o.has(x.rgb) || x.alpha === "00" || (g = N(`#${x.rgb}${x.alpha}`, i.color), w = e[y]);
    }
    if (!g || S(N(h, g), g) >= 1.5)
      continue;
    const f = Zt(h, g, bt);
    f.changed && (Ct(l, f.color), s.nudged++);
  }
  return s;
}
function Ct(t, e) {
  O.has(t.type) ? t.color = e : t.colors = [e];
}
function Ho(t, e, i) {
  const o = { filled: 0, fieldPages: 0, unresolved: 0, fonts: 0, recoloured: 0, backgrounds: 0, readability: ie() };
  if (i.fields) {
    const s = Xt(e), a = /* @__PURE__ */ new Set();
    t.forEach((l, d) => {
    ...
    }), o.fieldPages = a.size, o.unresolved = eo(t.flatMap((l) => l.layers)).filter($t).length;
  }
  const r = Y(e.fonts.heading), n = Y(e.fonts.body);
  if (i.fonts && (r || n))
    for (const s of t) {
      const a = Mo(s.global);
      for (const l of s.layers) {
        if (!St(l))
          continue;
        const h = qt(l) || Number(l.fontSize) >= a ? r || n : n || r;
        h && ko(l, h) && o.fonts++;
      }
    }
  if (i.colors && e.colors.length) {
    const s = vo(t).slice(0, e.colors.length), a = /* @__PURE__ */ new Map();
    if (s.forEach((l, d) => {
      ...
      for (const l of t) {
        const d = zo(l.layers, l.global, e);
...
function $(t, e) {
  ...
  return Ho(i.layouts, Kt(e), { fields: !0, fonts: !0, colors: !0 }), i;
}
const I = 110, { width: oe, height: re } = R, p = oe - I * 2, v = re - 96, ne = ["title", "statement", "content", "two-column", "media"];
function gt(t) {
...
function k(t, e, i) {
  ...
  if (!o)
    return null;
  const r = Rt(o, { fontFamily: i.font.value, fontSize: i.size, lineHeight: i.lineHeight, letterSpacing: i.tracking, bold: (i.weight || 400) >= 600 }, { width: e.width, height: e.height, minFontSize: i.minSize, maxLines: i.maxLines });
  if (!r.lines.length)
    return null;
  const n = Et(r, i.lineHeight);
  return {
    widget: It({
      left: e.left,
      top: e.top,
...
function yt(t, e, i, o) {
  const r = [], n = Math.round(o * 0.75), s = Math.round(o * 0.35), a = Math.round(o * 1.3);
  let l = i.top;
  ...
  for (const h of e) {
    ...
    if (!c || c.bottom > d)
      break;
    r.push(b(i.left + Math.round(o * 0.35), l + Math.round(o * 0.52), Math.round(o * 0.3), Math.round(o * 0.3), t.accent, 999)), r.push(c.widget), l = c.bottom + s;
    for (const g of h.sub || []) {
      ...
      if (!w || w.bottom > d)
        break;
      r.push(b(i.left + a, l + Math.round(o * 0.5), Math.round(o * 0.5), 2, t.muted)), r.push(w.widget), l = w.bottom + s;
    }
    l += n - s;
  }
  return r;
...
function H(t, e) {
  const i = b(I, v, p, 2, t.rule), o = e("{{school.name|upper}}"), r = It({
    left: I,
    top: v + 22,
    width: p,
    height: 32,
...
function Wo(t, e, i) {
  const o = [], r = K(t, e.kicker, 110);
  r && o.push(r.widget), o.push(b(I, 190, 150, 8, t.accent));
  const n = k(
    e.title,
    { left: I, top: 260, width: Math.round(p * 0.84), height: 400 },
    {
      font: t.display,
      size: 112,
      minSize: 52,
  ...
  const a = k(
    e.sub,
    { left: I, top: s, width: Math.round(p * 0.68), height: v - 40 - s },
    {
      font: t.body,
      size: 38,
      minSize: 26,
...
function Go(t, e, i) {
  const o = [b(I, 250, 10, 420, t.accent)], r = I + 70, n = p - 70, s = K(t, e.kicker, 110);
  s && o.push(s.widget);
  const a = k(
    e.title || e.callout,
  ...
  const d = k(
    e.sub,
    { left: r, top: l, width: Math.round(n * 0.7), height: v - 40 - l },
    {
      font: t.body,
      size: 32,
      minSize: 22,
...
function se(t, e, i, o = p) {
  const r = K(t, e.kicker, 110, I, o);
  let n = r ? r.bottom + 14 : 110;
  r && i.push(r.widget);
  const s = k(
    e.title,
    { left: I, top: n, width: o, height: 200 },
    {
      font: t.display,
      size: 62,
      minSize: 34,
  ...
  return s && (i.push(s.widget), n = s.bottom + 26), i.push(b(I, n, o, 2, t.rule)), n + 40;
}
function le(t, e, i) {
  const o = [];
  let r = se(t, e, o);
  const n = k(
    e.sub,
    { left: I, top: r, width: Math.round(p * 0.8), height: 130 },
    {
      font: t.body,
      size: 30,
      minSize: 22,
  ...
  const s = e.callout ? 130 : 0;
  if (o.push(...yt(t, e.bullets, { left: I, top: r, width: p, height: v - 40 - s - r }, 32)), e.callout) {
    const a = v - 40 - s;
    o.push(b(I, a, p, s - 16, t.accent, 8));
    const l = k(
      e.callout,
      { left: I + 34, top: a + 28, width: p - 68, height: s - 72 },
      {
        font: t.body,
        size: 30,
        minSize: 20,
...
function To(t, e, i) {
  const o = [], r = se(t, e, o), n = 80, s = Math.round((p - n) / 2), a = [
    { left: I, head: e.columnHeads[0] || null, bullets: e.bullets },
    { left: I + s + n, head: e.columnHeads[1] || null, bullets: e.bulletsRight }
  ];
  o.push(b(I + s + Math.round(n / 2), r, 2, v - 40 - r, t.rule));
  for (const l of a) {
    let d = r;
    const h = k(
      l.head,
      { left: l.left, top: d, width: s, height: 90 },
      {
        font: t.display,
        size: 34,
        minSize: 24,
    ...
    h && (o.push(h.widget), d = h.bottom + 24), o.push(...yt(t, l.bullets, { left: l.left, top: d, width: s, height: v - 40 - d }, 28));
  }
  return [...o, ...H(t, i)];
}
function Ro(t, e, i) {
  const o = [], r = Math.round(p * 0.46), n = { left: I + r + 70, top: 110, width: p - r - 70, height: v - 150 };
  e.image?.url ? o.push(io(n.left, n.top, n.width, n.height, e.image)) : o.push(b(n.left, n.top, n.width, n.height, t.rule, 6));
  const s = K(t, e.kicker, 130, I, r);
  let a = s ? s.bottom + 16 : 130;
  s && o.push(s.widget);
  const l = k(
    e.title,
    { left: I, top: a, width: r, height: 300 },
    {
      font: t.display,
      size: 62,
      minSize: 32,
  ...
  const d = k(
    e.sub,
    { left: I, top: a, width: r, height: 200 },
    {
      font: t.body,
      size: 30,
      minSize: 22,
  ...
  return d && (a = d.bottom + 30), d && o.push(d.widget), o.push(...yt(t, e.bullets, { left: I, top: a, width: r, height: v - 40 - a }, 26)), [...o, ...H(t, i)];
}
const Eo = {
  title: Wo,
  statement: Go,
...
function ae(t, e, i = (o) => o) {
  const o = Eo[t.layout] || le, r = t.title?.trim() || t.kicker?.trim() || "Slide";
  return { global: Qt(r.slice(0, 60), oe, re, e.paper, t.notes), layers: o(e, t, i) };
}
function e0(t, e = {}) {
  const i = Wt(e.theme), o = F(e.brand), r = Array.isArray(t?.slides) ? t.slides : [], n = (r.length ? r : [gt("title")]).map((a) => ae({ ...gt(a?.layout || "content"), ...a }, i, o)), s = { format: "design-studio/v1", title: String(t?.title || "Untitled deck"), layouts: n };
  return e.brand ? $(s, e.brand) : s;
}
const de = [
  {
    id: 19,
...
const i0 = de.filter((t) => t.type === "svg").map((t) => t.title);
function he(t) {
  return !!t && B.has(String(t).trim().toLowerCase());
}
function xt(t, e, i, o, r) {
  const n = B.get(String(t).trim().toLowerCase());
  return n ? {
    name: n.title,
    type: "w-svg",
    uuid: Q(),
...
}, ce = ["direction", "icon", "statement", "number", "notice"];
function ut(t) {
  return { layout: t, icon: null, eyebrow: null, badge: null, head: "", sub: null, foot: null };
}
function Po(t) {
  const e = Nt[t?.size] || Nt.letter;
  return t?.orientation === "LANDSCAPE" ? { width: e.height, height: e.width } : { ...e };
}
function Uo(t, e) {
...
function W(t, e, i, o, r) {
  const { W: n, H: s, M: a, content: l } = e, d = [], h = o ? t.paper : t.muted, c = Math.round(s * 0.07), g = M(
    i.eyebrow ? i.eyebrow.toUpperCase() : null,
    { left: a, top: c, width: l, height: Math.round(s * 0.05) },
    {
      font: t.eyebrow,
  ...
  d.push(b(a, w, l, 3, o ? t.paper : t.rule));
  const f = M(
    r(i.foot || "{{school.name}} · {{school.phone}}"),
    { left: a, top: w + Math.round(s * 0.018), width: l, height: Math.round(s * 0.05) },
    {
      font: t.eyebrow,
  ...
  return f && d.push(f.widget), d;
}
function A(t, e, i) {
  return { left: t.M, top: Math.round(e), width: t.content, height: Math.round(i) };
}
function Yo(t, e, i, o) {
  const { W: r, H: n, M: s, content: a } = e, l = [];
  l.push(b(0, 0, r, Math.round(n * 0.055), t.accent));
  const d = M(i.head, A(e, n * 0.2, n * 0.34), {
    font: t.display,
  ...
  let h = Math.round(n * 0.56);
  d && (l.push(d.widget), h = d.bottom + Math.round(n * 0.035));
  const c = xt("arrow right", s + Math.round((a - r * 0.22) / 2), h, Math.round(r * 0.22), t.accent);
  c && (l.push(c), h += Math.round(r * 0.22) + Math.round(n * 0.02));
  const g = M(i.sub, A(e, h, n * 0.86 - h), {
    font: t.body,
...
function Jo(t, e, i, o) {
  const { W: r, H: n, M: s, content: a } = e, l = [], d = Math.round(r * 0.34);
  let h = Math.round(n * 0.17);
  const c = he(i.icon) ? xt(i.icon, s + Math.round((a - d) / 2), h, d, t.accent) : null;
  c && (l.push(c), h += d + Math.round(n * 0.04));
  const g = M(i.head, A(e, h, n * 0.28), {
    font: t.display,
  ...
  g && (l.push(g.widget), h = g.bottom + Math.round(n * 0.03));
  const w = M(i.sub, A(e, h, n * 0.86 - h), {
    font: t.body,
  ...
  return w && l.push(w.widget), { layers: [...l, ...W(t, e, i, !1, o)], background: t.paper };
}
function Fo(t, e, i, o) {
  const { W: r, H: n } = e, s = [], a = M(i.head, A(e, n * 0.24, n * 0.4), {
    font: t.display,
  ...
  a && (s.push(a.widget), l = a.bottom + Math.round(n * 0.035));
  const d = M(i.sub, A(e, l, n * 0.86 - l), {
    font: t.body,
  ...
  return d && s.push(d.widget), { layers: [...s, ...W(t, e, i, !0, o)], background: t.accent };
}
function Qo(t, e, i, o) {
  const { W: r, H: n, M: s, content: a } = e, l = [], d = M(i.badge || i.head, A(e, n * 0.19, n * 0.3), {
    font: t.display,
  ...
  let h = Math.round(n * 0.5);
  d && (l.push(d.widget), h = d.bottom + Math.round(n * 0.03)), l.push(b(s + Math.round(a * 0.35), h, Math.round(a * 0.3), 6, t.rule)), h += Math.round(n * 0.035);
  const c = M(i.badge ? i.head : i.sub, A(e, h, n * 0.2), {
    font: t.display,
  ...
  c && (l.push(c.widget), h = c.bottom + Math.round(n * 0.025));
  const g = M(i.badge ? i.sub : null, A(e, h, n * 0.86 - h), {
    font: t.body,
...
function ge(t, e, i, o) {
  const { W: r, H: n, M: s, content: a } = e, l = [];
  l.push(b(s, Math.round(n * 0.135), Math.round(a * 0.18), 10, t.accent));
  const d = M(i.head, A(e, n * 0.18, n * 0.26), {
    font: t.display,
  ...
  let h = Math.round(n * 0.46);
  d && (l.push(d.widget), h = d.bottom + Math.round(n * 0.03));
  const c = he(i.icon) ? xt(i.icon, e.W - s - Math.round(r * 0.14), Math.round(n * 0.17), Math.round(r * 0.14), t.accentSoft) : null;
  c && l.push(c);
  const g = M(i.sub, A(e, h, n * 0.86 - h), {
    font: t.body,
    size: Math.round(r * 0.04),
...
function ue(t, e, i, o = (r) => r) {
  const r = Uo(i.width, i.height), n = Vo[t.layout] || ge, { layers: s, background: a } = n(e, r, t, o), l = t.head?.trim() || t.eyebrow?.trim() || "Sign";
  return { global: Qt(l.slice(0, 60), i.width, i.height, a), layers: s };
}
function o0(t, e = {}) {
  const i = Gt(e.theme), o = F(e.brand), r = Po(t || { orientation: "PORTRAIT", size: "letter" }), n = Array.isArray(t?.signs) ? t.signs : [], s = (n.length ? n : [ut("notice")]).map((l) => ue({ ...ut(l?.layout || "notice"), ...l }, i, r, o)), a = { format: "design-studio/v1", title: n[0]?.head?.trim() || "Untitled sign", layouts: s };
  return e.brand ? $(a, e.brand) : a;
}
function Oo(t) {
  const e = Yt(t.text), i = e.find((o) => $t(o));
...
function ft(t) {
  ...
  return i && Math.abs(Number(e.width) / Number(e.height) - R.width / R.height) < 0.06 ? "slides" : !i && Math.abs(Number(e.width) / Number(e.height) - lt.width / lt.height) < 0.25 ? "poster" : i ? "slides" : "poster";
}
function r0(t) {
  ...
  return {
    ...
    pages: e.map((i, o) => {
      ...
      return {
        ...
        texts: r.filter((s) => s.type === "w-text" && !s.hidden).map((s) => ({ id: String(s.uuid), role: Oo(s), text: wt(s.text) })).filter((s) => s.text.length > 0),
        images: r.filter((s) => s.type === "w-image" && !s.hidden).map((s) => ({ id: String(s.uuid), alt: Xo(s) })),
        notes: n || null
      };
    })
  };
...
function _o(t) {
  const e = String(t ?? "");
  return /<\/?[a-z][^>]*>/i.test(e) ? e : V(e);
}
function zt(t, e) {
  return e ? $({ format: "design-studio/v1", title: "", layouts: [t] }, e).layouts[0] : t;
}
function qo(t, e, i, o) {
  const r = String(i.bullets || "").split(`
`).map((s) => s.trim()).filter(Boolean).map((s) => ({ text: s, sub: [] }));
  if (ft(t) === "poster") {
    if (!ce.includes(e))
      return null;
    const s = t.layouts[0]?.global, a = { width: Number(s?.width) || 1275, height: Number(s?.height) || 1650 }, l = {
      ...ut(e),
      icon: i.icon || null,
    ...
    return zt(ue(l, Gt(i.theme), a, F(o)), o);
  }
  if (!ne.includes(e))
    return null;
  const n = {
...
`).map((s) => s.trim()).filter(Boolean).map((s) => ({ text: s, sub: [] }))
  ...
  return zt(ae(n, Wt(i.theme), F(o)), o);
}
function n0(t, e, i = {}) {
  let o = Ko(t);
  const r = [], n = Array.isArray(e) ? e : [];
  for (const s of n) {
    if (!s || typeof s != "object") {
      r.push({ op: s, reason: "That is not an operation." });
      continue;
    }
    switch (s.op) {
      case "setText": {
        const a = Lt(o, s.id);
        if (!a) {
          r.push({ op: s, reason: `No widget with id ${s.id}.` });
          break;
        }
        if (a.type !== "w-text") {
          r.push({ op: s, reason: `Widget ${s.id} is a ${a.type}, which holds no text.` });
          break;
        }
        a.text = _o(s.text);
        break;
      ...
      case "setImage": {
        ...
        if (a.type !== "w-image") {
          r.push({ op: s, reason: `Widget ${s.id} is a ${a.type}, which holds no picture.` });
          break;
        }
        if (!s.url) {
          r.push({ op: s, reason: "A picture needs a url." });
          break;
        }
        const l = a.width / a.height, d = s.width && s.height ? s.width / s.height : l;
        a.imgUrl = s.url;
      ...
      case "addPage": {
        if (o.layouts.length >= jt) {
          r.push({ op: s, reason: `A design holds at most ${jt} pages.` });
          break;
        }
        const a = Number(s.after);
        if (!Number.isInteger(a) || a < -1 || a > o.layouts.length - 1) {
          r.push({ op: s, reason: `There is no page ${s.after} to add after.` });
          break;
        }
        const l = qo(o, String(s.kind), s.fields || {}, i.brand);
        if (!l) {
          r.push({ op: s, reason: `“${s.kind}” is not a page this design can hold. Try one of: ${$o(ft(o) === "poster" ? "poster" : "slides").join(", ")}.` });
          break;
        }
      ...
      case "removePage": {
        const a = Number(s.index);
        if (!Number.isInteger(a) || a < 0 || a >= o.layouts.length) {
          r.push({ op: s, reason: `There is no page ${s.index}.` });
          break;
        }
        if (o.layouts.length === 1) {
          r.push({ op: s, reason: "A design has to keep one page." });
          break;
        }
      ...
      case "movePage": {
        const a = Number(s.from), l = Number(s.to);
        if (!Number.isInteger(a) || a < 0 || a >= o.layouts.length) {
          r.push({ op: s, reason: `There is no page ${s.from}.` });
          break;
        }
        if (!Number.isInteger(l) || l < 0 || l >= o.layouts.length) {
          r.push({ op: s, reason: `Page ${s.from} cannot go to ${s.to}; there are ${o.layouts.length} pages.` });
          break;
        }
      ...
      case "applyBrand": {
        if (!i.brand) {
          r.push({ op: s, reason: "No brand kit was given to apply." });
          break;
        }
        o = $(o, i.brand);
        break;
      ...
      default:
        r.push({ op: s, reason: `“${s.op}” is not an operation.` });
    }
  }
  return { doc: o, rejected: r };
}
export {
  ne as DECK_PAGE_KINDS,
  i0 as ICON_KEYS,
  Ei as POSTER_PACK_KEYS,
  lt as POSTER_PAGE,
  ce as SIGN_PAGE_KINDS,
  R as SLIDE_PAGE,
  Ri as SLIDE_THEME_KEYS,
  $ as applyBrand,
  n0 as applyOps,
  ut as blankSign,
  gt as blankSlide,
