...
function Be(t, e) {
  ...
  return ze.find((n) => Math.abs(n.mm[0] - r) <= vt && Math.abs(n.mm[1] - s) <= vt) ?? null;
}
function Ze(t, e) {
  const i = Be(t, e);
  return i ? `${i.name} ${Number(e) >= Number(t) ? "portrait" : "landscape"}` : null;
}
const P = "#ffffffff", He = "#000000ff", We = 0.6, St = 0.02, Te = 1.2, Jt = 96, Ge = 0.179;
function U(t) {
  if (typeof t != "string")
    return null;
...
function Ee(t) {
  return Ze(t.width, t.height) ? G : Jt;
}
function Re(t, e, i) {
  const o = (Number(t) || 0) * (Jt / Ee(i));
...
const Je = "201", Fe = "Editorial slide — cover", Qe = 1920, Oe = 1080, Ve = "slide-themes", Xe = {
  ...
  brand: Xe,
  data: $e
}, Ke = "206", qe = "Swiss slide — cover", ti = 1920, ei = 1080, ii = "slide-themes", oi = {
  colors: {
    e4322b: "primary"
  }
  ...
  brand: ci,
  data: gi
}, wi = "216", fi = "Dark slide — cover", Ii = 1920, yi = 1080, xi = "slide-themes", pi = {
  colors: {
    "46cdb4": "primary"
  }
  ...
  brand: Yi,
  data: Ji
}, Qi = "107", Oi = "Science Fair poster", Vi = 1275, Xi = 1650, $i = "school-events", _i = {
  colors: {
    "2f6b3a": "primary"
  }
...
function Ot(t) {
  ...
  return st.has(e) || st.set(e, Qt(e, oo[e])), st.get(e);
}
function Vt(t) {
  const e = eo.includes(String(t)) ? t : "navy";
  return lt.has(e) || lt.set(e, Qt(e, ro[e])), lt.get(e);
}
...
function ut(t, e) {
  const i = e.fontFamily ? ho[e.fontFamily] : void 0, o = e.fontFamily && co[e.fontFamily] || 1;
  let r = 0;
  if (i !== void 0) {
    for (const n of t)
      r += i;
  ...
  return (r * e.fontSize * o * s + (e.letterSpacing || 0) * t.length) * go;
}
function Nt(t, e, i) {
  const o = [];
...
function Xt(t, e, i) {
  ...
  const r = (h) => ({ ...e, fontSize: h, letterSpacing: (e.letterSpacing || 0) * (h / e.fontSize) }), s = (h, c) => h.reduce((g, u) => Math.max(g, ut(u, c)), 0), n = Math.max(6, Math.min(i.minFontSize, e.fontSize));
  for (let h = e.fontSize; h >= n; h -= 1) {
    const c = r(h), g = Nt(o, i.width, c), u = Math.min(jt(i.height, h, e.lineHeight), i.maxLines || Number.MAX_SAFE_INTEGER);
    if (g.length <= u && s(g, c) <= i.width)
      return { text: o, fontSize: h, lines: g, truncated: !1 };
  }
  const a = r(n), l = Math.min(jt(i.height, n, e.lineHeight), i.maxLines || Number.MAX_SAFE_INTEGER), d = Nt(o, i.width, a).slice(0, l);
  for (; d.length && ut(`${d[d.length - 1]}…`, a) > i.width; ) {
    const h = d[d.length - 1], c = h.split(" ");
    if (c.length > 1) {
      c.pop(), d[d.length - 1] = c.join(" ");
      continue;
    }
    if (h.length <= 1) {
      d.pop();
...
const wo = /^(address|blockquote|div|dl|dd|dt|h[1-6]|li|ol|p|pre|table|td|th|tr|ul)$/, fo = {
  amp: "&",
  lt: "<",
  gt: ">",
  quot: '"',
...
`).replace(/<\/?([a-z][a-z0-9]*)\b[^>]*>/gi, (i, o) => wo.test(String(o).toLowerCase()) ? `
` : "");
  return O(e).replace(/\n{3,}/g, `
...
function Io(t) {
  return !t || !t.includes("{{") ? !1 : pt(t).search(_t) !== -1;
}
function Kt(t) {
  if (!t || !t.includes("{{"))
    return [];
  const e = [], i = /* @__PURE__ */ new Set();
  for (const o of pt(t).matchAll(_t)) {
...
function J(t) {
  return t ? mo.find((e) => e.id === t) : void 0;
}
function So(t, e) {
  switch (j(e)) {
    case "upper":
...
const Do = /^#?([0-9a-f]{6})([0-9a-f]{2})?$/i;
function Z(t) {
  const e = Do.exec((t || "").trim());
...
function le(t, e, i) {
  const o = Z(e), r = Z(i);
  return !o || !r || o.alpha === "00" || o.rgb === r.rgb && o.alpha === r.alpha ? t : Co(t, o, r);
}
function No(t, e) {
...
}, zo = "circle at 50% 50%", Bo = (t) => /^(linear|radial)-gradient\(/.test(t?.trim() || "");
function Zo(t, e, i) {
  const o = i.map((r) => `${r.color} ${r.offset * 100}%`).join(",");
  return t === "radial" ? `radial-gradient(${zo}, ${o})` : `linear-gradient(${e}deg, ${o})`;
}
function Ho(t) {
  const e = [];
  let i = 0, o = 0;
...
function at(t) {
  ...
  return o.forEach((a, l) => {
    ...
    n.push({ color: Wo(h.trim()), offset: c });
...
function Eo(t, e) {
  return t.fontClass?.value === e.value ? !1 : (t.fontClass = Go(e), t.fontFamily = e.value, !0);
}
function m(t) {
  if (typeof t != "string")
    return null;
...
function Ro(t) {
  return No(t.textEffects, t.color).map((e) => ({
    kind: "layer",
    read: () => e.value,
    write: (i) => {
...
const wt = /* @__PURE__ */ new Set([...$, "w-svg"]);
function z(t) {
...
function he(t, e, i, o) {
  ...
  for (let h = 0; h < e.length; h++) {
    const c = e[h];
    h === t || c.hidden || c.isContainer || F(i[t], i[h]) && (l && !F(i[h], l) || Uo(z(c), s, n) && (a = c, l = i[h]));
  }
  if (!a)
    return o;
  if (!wt.has(a.type))
...
function Vo(t, e, i) {
  ...
  if (!r.size)
    return o;
  const s = Oo(t), n = Jo(t), a = Fo(e), l = t.map((c, g) => c.type === "w-text" && !c.hidden ? he(g, t, n, a) : null), d = /* @__PURE__ */ new Set();
  for (const c of l)
    c?.uuid && d.add(c.uuid);
  for (let c = 0; c < t.length; c++) {
    ...
    if (!y && !w)
      continue;
    const x = Pe(Number(g.fontSize) || 0, ae(g), e), Ne = N(`#${u.rgb}${u.alpha}`, I.color);
    if (S(Ne, I.color) >= x)
      continue;
    const tt = `#${u.rgb}${u.alpha}`;
    if (_(u.rgb)) {
      const it = gt(I.color, [tt, P, s]);
      it !== tt && (L(g, `#${m(it).rgb}${u.alpha}`), o.swapped++), S(it, I.color) < x && o.unreadable++;
      continue;
    }
    const D = Ft(tt, I.color, x);
    if (D.met) {
      D.changed && (L(g, D.color), o.adjusted++);
      continue;
    }
    const et = gt(I.color, [D.color, P, s]);
    et === D.color ? D.changed && (L(g, D.color), o.adjusted++) : (L(g, `#${m(et).rgb}${u.alpha}`), o.swapped++), S(et, I.color) < x && o.unreadable++;
  }
  const h = $o(t, n, a, r, s, d);
...
function $o(t, e, i, o, r, s) {
  ...
  for (let a = 0; a < t.length; a++) {
    const l = t[a];
    if (l.hidden || l.isContainer || !wt.has(l.type) || l.type === "w-svg" && l.colors?.length !== 1)
      continue;
    const d = m(ft(l));
    if (!d || d.alpha === "00")
      continue;
    ...
    if (_(d.rgb)) {
      const y = he(a, t, e, i);
      if (!y || !o.has(y.rgb) || s.has(l.uuid) || zt(c) > Xo * zt(y.bounds) || S(N(h, y.color), y.color) >= Dt)
        continue;
      const w = gt(y.color, [h, P, r]);
      if (w === h)
        continue;
      Bt(l, `#${m(w).rgb}${d.alpha}`), n.swapped++;
      continue;
    }
    if (!o.has(d.rgb))
      continue;
    ...
    for (let y = 0; y < t.length; y++) {
      const w = t[y];
      if (y === a || w.hidden || !wt.has(w.type) || !F(e[a], e[y]) || u && !F(e[y], u) || !Yo(z(w), c))
        continue;
      const x = m(ft(w));
    ...
    if (!g || S(N(h, g), g) >= 1.5)
      continue;
    const I = Ft(h, g, Dt);
    I.changed && (Bt(l, I.color), n.nudged++);
  }
  return n;
}
function Bt(t, e) {
...
function _o(t, e, i) {
  ...
  if (i.colors && e.colors.length) {
    const n = Po(t).slice(0, e.colors.length), a = /* @__PURE__ */ new Map();
    if (n.forEach((l, d) => {
      ...
      for (const l of t) {
        const d = Vo(l.layers, l.global, e);
...
function K(t, e) {
  ...
  return _o(i.layouts, se(e), { fields: !0, fonts: !0, colors: !0 }), i;
}
...
function e0(t, e, i) {
  ...
  e.image?.url ? o.push(po(s.left, s.top, s.width, s.height, e.image)) : o.push(b(s.left, s.top, s.width, s.height, t.rule, 6));
...
function ye(t, e, i = (o) => o) {
  const o = i0[t.layout] || Ie, r = t.title?.trim() || t.kicker?.trim() || "Slide";
  return { global: ee(r.slice(0, 60), ge, ue, e.paper, t.notes), layers: o(e, t, i) };
}
function Q0(t, e = {}) {
  const i = Ot(e.theme), o = Q(e.brand), r = Array.isArray(t?.slides) ? t.slides : [], s = (r.length ? r : [It("title")]).map((a) => ye({ ...It(a?.layout || "content"), ...a }, i, o)), n = { format: "design-studio/v1", title: String(t?.title || "Untitled deck"), layouts: s };
...
const O0 = xe.filter((t) => t.type === "svg").map((t) => t.title);
function pe(t) {
  return !!t && B.has(String(t).trim().toLowerCase());
}
function bt(t, e, i, o, r) {
...
function r0(t) {
  const e = o0[String(t?.size)], i = Zt[e ? e.size : t?.size] || Zt.letter;
...
function s0(t, e, i, o) {
  ...
  const c = bt("arrow right", n + Math.round((a - r * 0.22) / 2), h, Math.round(r * 0.22), t.accent);
...
function l0(t, e, i, o) {
  ...
  const c = pe(i.icon) ? bt(i.icon, n + Math.round((a - d) / 2), h, d, t.accent) : null;
...
function Me(t, e, i, o) {
  ...
  const c = pe(i.icon) ? bt(i.icon, e.W - n - Math.round(r * 0.14), Math.round(s * 0.17), Math.round(r * 0.14), t.accentSoft) : null;
  c && l.push(c);
  const g = M(i.sub, A(e, h, s * 0.86 - h), {
    font: t.body,
    size: Math.round(r * 0.04),
...
function be(t, e, i, o = (r) => r) {
  const r = n0(i.width, i.height), s = h0[t.layout] || Me, { layers: n, background: a } = s(e, r, t, o), l = t.head?.trim() || t.eyebrow?.trim() || "Sign";
  return { global: ee(l.slice(0, 60), i.width, i.height, a), layers: n };
}
function V0(t, e = {}) {
  const i = Vt(e.theme), o = Q(e.brand), r = r0(t || { orientation: "PORTRAIT", size: "letter" }), s = Array.isArray(t?.signs) ? t.signs : [], n = (s.length ? s : [yt("notice")]).map((l) => be({ ...yt(l?.layout || "notice"), ...l }, i, r, o)), a = { format: "design-studio/v1", title: s[0]?.head?.trim() || "Untitled sign", layouts: n };
  return e.brand ? K(a, e.brand) : a;
}
function c0(t) {
  const e = Kt(t.text), i = e.find((o) => ne(o));
  ...
  return {
    ...
    pages: e.map((i, o) => {
      ...
      return {
        ...
        texts: r.filter((n) => n.type === "w-text" && !n.hidden).map((n) => ({ id: String(n.uuid), role: c0(n), text: pt(n.text) })).filter((n) => n.text.length > 0),
        images: r.filter((n) => n.type === "w-image" && !n.hidden).map((n) => ({ id: String(n.uuid), alt: g0(n) })),
        notes: s || null
      };
    })
  };
}
const ke = ["href", "color", "bold", "italic", "underline", "strike"], Ht = /^(ADDRESS|ARTICLE|BLOCKQUOTE|DIV|DL|DD|DT|FOOTER|H[1-6]|HEADER|LI|OL|P|PRE|SECTION|TABLE|TD|TH|TR|UL)$/, Wt = /^(HEAD|LINK|META|NOSCRIPT|SCRIPT|STYLE|TEMPLATE|TITLE)$/, Tt = 3, Gt = 1, Ae = 64;
...
function xt(t) {
  return t.replace(/[&<>"\u00a0]/g, (e) => u0[e]);
}
const w0 = /^#([0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i, f0 = /^rgba?\(\s*([^)]+)\)$/i, T = (t) => Math.max(0, Math.min(255, Math.round(t))).toString(16).padStart(2, "0");
function Et(t) {
  if (!t)
    return;
  const e = t.trim(), i = e.match(w0);
  if (i) {
...
const I0 = /^([a-z][a-z0-9+.-]*):/i, y0 = /* @__PURE__ */ new Set(["http", "https", "mailto", "tel"]);
function x0(t) {
  const e = (t ?? "").trim();
  if (!e)
    return;
  const i = e.match(I0)?.[1].toLowerCase();
  if (i)
    return y0.has(i) ? e : void 0;
  if (e.startsWith("//"))
    return "https:" + e;
  if (!/\s/.test(e))
    return "https://" + e;
...
function m0(t, e) {
  ...
  switch (t.tagName) {
    ...
    case "FONT": {
      const r = Et(t.getAttribute?.("color"));
      r && (i.color = r);
      break;
    }
  }
  ...
  const r = () => {
    ...
    for (; ; ) {
      for (let c = d.nextSibling; c; c = c.nextSibling)
        if (c.nodeType === Tt && c.data || c.nodeType === Gt && !Wt.test(c.tagName ?? ""))
          return !1;
      const h = d.parentNode;
      if (!h || h === t || Ht.test(h.tagName ?? ""))
        return !0;
      d = h;
    }
  }, n = (l, d) => {
    ...
    h && p0(h, d) ? h.text += l : i.push({ text: l, ...d }), o = !0;
  }, a = (l, d, h = 0) => {
    if (!(h > Ae))
...
function b0(t, e = "none") {
  if (e === "bullet" || e === "number") {
    const o = e === "number" ? "ol" : "ul", r = t.map((s) => `<li>${Rt(s).trim() ? Pt(s) : "<br>"}</li>`);
    return `<${o}>${r.join("")}</${o}>`;
  }
  const i = t.map(Pt).join("<br>");
  return t.length > 1 && !Rt(t[t.length - 1]) ? i + "<br>" : i;
}
const k0 = 3, De = 1, A0 = Ae, v0 = /* @__PURE__ */ new Set(["AREA", "BASE", "BR", "COL", "EMBED", "HR", "IMG", "INPUT", "LINK", "META", "PARAM", "SOURCE", "TRACK", "WBR"]), S0 = /* @__PURE__ */ new Set(["SCRIPT", "STYLE", "TEXTAREA", "TITLE", "XMP", "IFRAME", "NOEMBED", "NOFRAMES"]), D0 = {
  LI: /^LI$/,
  P: /^P$/,
  DD: /^(DD|DT)$/,
  DT: /^(DD|DT)$/,
...
function C0(t) {
  return { nodeType: k0, data: t, childNodes: [], nextSibling: null, parentNode: null };
}
function N0(t) {
  if (!t)
    return null;
...
function Ce(t) {
  ...
  for (; a < n.length; ) {
    ...
    const g = z0(h, L0(d.attrs));
    if (r(g), S0.has(h)) {
      const u = n.toUpperCase().indexOf(`</${h}`, a);
      a = u < 0 ? n.length : n.indexOf(">", u) + 1 || n.length;
      continue;
    }
    !v0.has(h) && !d.selfClosing && i.length < A0 && i.push(g);
  }
  return e;
}
function W0(t, e = "none") {
  return b0(ve(Ce(String(t ?? ""))), e);
}
function $0(t) {
  return ve(Ce(String(t ?? ""))).map((e) => e.map((i) => i.text).join("")).join(`
`);
}
const _0 = ["w-text", "w-image", "w-svg", "w-rect", "w-ellipse", "w-polygon", "w-path", "w-group", "w-qrcode", "w-table"], K0 = "page", q0 = {
...
}, T0 = /^[A-Za-z0-9 _-]{1,64}$/, G0 = {
  "w-text": ["fontClass.value"],
  "w-table": ["fontClass.value"]
};
function E0(t, e) {
  return typeof e != "string" ? !1 : t === "fontClass.value" ? T0.test(e) : !0;
}
function R0(t, e) {
  const i = e.split(".");
  let o = t;
...
function P0(t) {
  ...
  for (const o of e.layouts || [])
    for (const r of o.layers || [])
      for (const s of G0[String(r.type)] || []) {
        const n = R0(r, s);
        if (!n)
          continue;
        const a = n.holder[n.key];
        a === void 0 || E0(s, a) || (i.dropped.push({ type: String(r.type), path: s, value: String(a).slice(0, 80) }), s === "fontClass.value" ? (delete r.fontClass, delete r.fontFamily) : delete n.holder[n.key]);
      }
...
`).map((n) => n.trim()).filter(Boolean).map((n) => ({ text: n, sub: [] }));
  if (kt(t) === "poster") {
    ...
    return Yt(be(l, Vt(i.theme), a, Q(o)), o);
  }
  if (!we.includes(e))
    return null;
  const s = {
...
`).map((n) => n.trim()).filter(Boolean).map((n) => ({ text: n, sub: [] }))
  ...
  return Yt(ye(s, Ot(i.theme), Q(o)), o);
}
function er(t, e, i = {}) {
  let o = Y0(t);
  ...
  for (const n of s) {
    ...
    switch (n.op) {
      ...
      case "setMarkup": {
        ...
        a.text = W0(String(n.html ?? ""), a.listStyle);
        break;
      }
      case "setImage": {
        const a = dt(o, n.id);
      ...
      case "addPage": {
        if (o.layouts.length >= Ut) {
          r.push({ op: n, reason: `A design holds at most ${Ut} pages.` });
          break;
        }
        ...
        const l = J0(o, String(n.kind), n.fields || {}, i.brand);
        if (!l) {
          r.push({ op: n, reason: `“${n.kind}” is not a page this design can hold. Try one of: ${U0(kt(o) === "poster" ? "poster" : "slides").join(", ")}.` });
          break;
        }
...
export {
  ...
  O0 as ICON_KEYS,
  tr as NESTED_URL_PATHS,
  K0 as PAGE_TYPE,
  eo as POSTER_PACK_KEYS,
  ht as POSTER_PAGE,
  T0 as SAFE_FONT_FAMILY,
  G0 as SANITISED_FIELDS,
  ...
  Ce as parseMarkup,
  Vt as posterPack,
  P0 as sanitizeFields,
  W0 as sanitizeMarkup,
  Ot as slideTheme
};
