...
if (!KINDS[kind] || !source) {
	console.error(
		'usage: node add-content.mjs <shapes|stickers|masks> <folder> [--replace]',
	)
	process.exit(1)
}
...
function templateColours(markup) {
	...
	const templated = markup.replace(
		/(fill|stroke)\s*=\s*"(#[0-9a-f]{3,8}|currentColor)"/gi,
		(_, attr, raw) => {
...
const existing = replace
	? []
	: JSON.parse(await fs.readFile(target, 'utf8').catch(() => '[]'))
let nextId = Math.max(0, ...existing.map((i) => Number(i.id) || 0)) + 1

const files = (await fs.readdir(source))
	.filter((f) => spec.exts.includes(path.extname(f).toLowerCase()))
	.sort()
...
await fs.writeFile(
	target,
	JSON.stringify([...added, ...existing], null, 2) + '\n',
)
console.log(
