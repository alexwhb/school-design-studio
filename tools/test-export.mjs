...
page.on('console', (m) => {
  if (m.type() === 'error' && !m.text().includes('ERR_CONNECTION_REFUSED')) errors.push('CONSOLE: ' + m.text().slice(0, 300))
})
...
// --- inspect ------------------------------------------------------------
execSync(`cd ${OUT} && unzip -o -q "${download.suggestedFilename()}" -d unpacked`)
const slides = (await fs.readdir(`${OUT}/unpacked/ppt/slides`)).filter((f) => f.endsWith('.xml'))
...
if (dims) {
  const EMU = 914400
  console.log(`slide size: ${(dims[1] / EMU).toFixed(2)}in x ${(dims[2] / EMU).toFixed(2)}in`)
}
