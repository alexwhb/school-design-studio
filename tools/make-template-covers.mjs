...
const templates = JSON.parse(await fs.readFile(LIST, 'utf8')).filter((item) => (PACK ? item.pack === PACK : Boolean(item.pack)) && (only.size === 0 || only.has(String(item.id))))
if (!templates.length) {
  console.error(`No ${PACK ? `"${PACK}"` : 'packed'} templates in list.json. Run the generator first.`)
  process.exit(1)
}
...
for (const template of templates) {
  ...
  const ready = await page
    .waitForFunction(
      () => {
        ...
        const drawn = canvas.querySelectorAll('.w-text, .w-svg, .w-image').length
  ...
  await page.locator('#page-design-canvas').screenshot({ path: path.join(OUT, file) })
  if (existsSync(DIST_OUT)) await fs.copyFile(path.join(OUT, file), path.join(DIST_OUT, file))
