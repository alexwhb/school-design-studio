...
export default {
  upload: async (file: File | Blob, options: Options, cb?: IQiniuSubscribeCb) => {
    const win = window
    let name = ''
    const suffix = file.type.split('/')[1] || 'png' // File extension
    if (!options.fullPath) {
      // const DT: any = await exifGetTime(file) // 照片时间
      const DT = new Date()
      const YM = `${dayjs(DT).format('YYYY')}/${dayjs(DT).format('MM')}/` // Group files by date
      const keyName = YM + new Date(DT).getTime()
      const prePath = options.prePath ? options.prePath + '/' : ''
      name = `${prePath}${keyName}` + `.${suffix}` // File name
    } else name = options.fullPath + `.${suffix}` // File name
    ...
    return new Promise((resolve: IQiniuSubscribeCb, reject: (err: string) => void) => {
      observable.subscribe({
        next: (result) => {
          cb?.(result) // result.total.percent -> Show progress
        },
        error: (e) => {
          reject(e)
        },
        complete: (result) => {
          resolve(result)
          // cb && cb(result) // result.total.percent -> Show progress
        },
      })
    })
  },
