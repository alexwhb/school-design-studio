...
export function resizePages(store: TWidgetStore, payload: TResizePagesPayload) {
  ...
  const indexes = payload.scope === 'all' ? store.dLayouts.map((_, index) => index) : [canvasStore.dCurrentPage]

  for (const index of indexes) {
    const layout = store.dLayouts[index]
    if (!layout) continue
  ...
  showPage(store, canvasStore.dCurrentPage)
}
