/*
 * What the brand kit does to a design, as arithmetic over pages.
 *
 * Three operations, all of them across the whole design rather than the page on
 * screen, for the same reason find-and-replace is: the school's name is on every
 * slide, and the one it misses is the one that gets printed.
 *
 * Nothing here reads the editor's store or touches a document, so the compose
 * entry can apply a school's brand to a design on a server and get exactly what
 * the Brand panel's Apply brand would have given it in the browser. `brand.ts`
 * beside this is the half that does know about the store: painting the
 * selection, dropping a field or the crest onto the page on screen.
 *
 ...
 */
import { brandFont, brandResolver, brandRoleIndex, isBrandField, type TBrandKit, type TTemplateBrand } from '@/common/methods/brandKitCore'
import { DECORATIVE_TARGET, INK, PAPER, adjustForContrast, composite, contrastRatio, contrastTarget, readableOn, relativeLuminance } from '@/common/methods/contrast'
...
import { fieldsInLayers, fillMarkup, hasFields } from '@/utils/mergeFieldsCore'
      ...
      if (page.backgroundGradient) {
        slots.push(
          ...paintSlots(
            ...
            (value) => (page.backgroundGradient = value),
          ),
        )
      } else {
        slots.push(
          ...paintSlots(
            ...
            (value) => (page.backgroundColor = value),
          ),
        )
      }
    }
      ...
      if (layer.type === 'w-text') {
        slots.push(
          ...paintSlots(
            'layer',
            () => layer.color,
            (value) => paintTextColor(layer, value),
          ),
        )
 ...
 */
function repairMarks(layers: TdWidgetData[], keys: number[][], backdrop: TSurface | null, brandRgbs: Set<string>, ink: string, carrying: Set<string>): { nudged: number; swapped: number } {
 ...
 */
export function applyBrandToLayouts(layouts: TdLayout[], kit: TBrandKit, options: TApplyBrandOptions): TApplyBrandOutcome {
  ...
  if (options.fields) {
    ...
    layouts.forEach((layout, index) => {
      for (const layer of layout.layers) {
        ...
        const next = fillMarkup(layer.text, resolve)
