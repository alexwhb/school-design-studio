 ...
 */
import { htmlToLines, lineText, linesToHtml, type TLineListStyle } from '@/utils/widgets/richText'
...
export function textToLines(html?: string): string[] {
  return htmlToLines(html).map(lineText)
}
...
export function applyListStyle(html: string | undefined, style: TListStyle): string {
  return linesToHtml(htmlToLines(html), style)
}
