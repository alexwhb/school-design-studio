...
export default function WRectStyle() {
  ...
  const corners = readCorners(active)
  const unlinked = isUnlinked(active)
  const limit = Math.round(maxRadius(active.width, active.height))
    ...
    <div className="ds-rect-style">
      <PanelSections value={activeNames} onChange={setActiveNames}>
        ...
        <PanelSection name="3" title="Corners">
          <CornerRadius corners={corners} unlinked={unlinked} maxValue={limit} onLinkChange={linkChange} onChange={radiusChange} />
        </PanelSection>
