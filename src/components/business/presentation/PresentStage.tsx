 ...
 */
const PresentStage = forwardRef<PresentModeHandle, PresentStageProps>(function PresentStage({ pages, currentPage, onExit, container }, ref) {
  const exitRef = useRef({ currentPage, onExit })
  exitRef.current = { currentPage, onExit }
  ...
  const close = useCallback(() => {
    ...
    clearTimeout(idleTimer.current)
    exitFullscreen()
    // Tell whoever opened it where the talk finished; the editor puts that
    // page on its canvas.
    exitRef.current.onExit?.(live.current.index)
  ...
  const open = useCallback(
    (startAt?: number) => {
      ...
      const start = clamp(startAt ?? exitRef.current.currentPage?.() ?? 0)
    ...
    <div ref={rootRef} className={cx('present', { 'present--idle': isIdle && !isOverview })} tabIndex={-1} onMouseMove={wake} onClick={onStageClick} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      ...
      >
        <div className="present__bar">
          ...
          <button type="button" className={cx('present__btn', 'present__btn--window', { 'is-on': !!viewMount })} title="Presenter view (S)" onClick={openPresenterView}>
            <svg viewBox="0 0 24 24" aria-hidden="true">
      ...
      {viewMount ? createPortal(<PresenterView pages={pages} />, viewMount) : null}
    </div>,
    container?.() ?? getPortalContainer() ?? document.body,
  )
})
