...
export default function Tooltip({ content, placement = 'bottom', effect = 'dark', showAfter = 0, disabled, children, popperClass }: TooltipProps) {
  if (disabled) return <>{children}</>
  return (
    <TooltipPrimitive.Root delayDuration={showAfter}>
      <TooltipPrimitive.Trigger asChild>{children}</TooltipPrimitive.Trigger>
      <TooltipPrimitive.Portal container={getPortalContainer()}>
        <TooltipPrimitive.Content side={placement} sideOffset={8} className={cx('el-popper', `is-${effect}`, 'ds-popper', popperClass || '')}>
          {content}
