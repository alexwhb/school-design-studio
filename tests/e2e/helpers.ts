...
export async function armShapeTool(page: Page, label: 'Rectangle' | 'Ellipse' | 'Polygon' | 'Line' | 'Arrow' | 'Pen') {
  ...
  if (tool !== 'pen') {
    ...
    await page.locator(`.tool-dock__shape[data-tool="${tool}"]`).click()
  } else {
    await page.locator('.tool-dock__item[data-tool="pen"]').click()
  }
  await page.waitForTimeout(300)
}
...
/**
 * One pixel of a PNG buffer, decoded by the browser rather than in Node — there
 * is no image decoder in the test process, and there is a perfectly good one on
...
/** Downloads whatever the given action produces, as bytes. */
export async function downloadBytes(page: Page, act: () => Promise<void>, timeout = 90000) {
