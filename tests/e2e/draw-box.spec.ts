...
test('dragging a corner grip rounds all four', async ({ page }) => {
  await armShapeTool(page, 'Rectangle')
  const board = await dragOnPage(page, { x: 60, y: 50 }, { x: 300, y: 220 })
  expect(await shapeRadius(page)).toBe('0px')
  ...
  const radius = Number.parseFloat((await shapeRadius(page))!)
  // One value back from CSS means all four corners agree.
  expect(await shapeRadius(page)).toBe(`${radius}px`)
...
test('Alt-dragging a grip rounds that corner alone, and the panel lets the four go', async ({ page }) => {
  ...
  const radius = (await shapeRadius(page))!.split(' ')
...
test('the panel’s chain puts the four corners back together', async ({ page }) => {
  ...
  await page.waitForTimeout(400)
  expect((await shapeRadius(page))!.split(' ').length).toBeGreaterThan(1)
...
test('drawing and rounding are each one step of undo', async ({ page }) => {
  ...
  await page.mouse.up()
  await page.waitForTimeout(400)
  const rounded = await shapeRadius(page)
  ...
  await page.waitForTimeout(500)
  await expect(page.locator(WIDGET)).toHaveCount(1)
  expect(await shapeRadius(page)).toBe(rounded)
})
